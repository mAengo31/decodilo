#!/usr/bin/env python3
"""Run the Lambda L3 direct-TCP learner/syncer experiment.
...
from decodilo.lambda_cloud.l3_direct_tcp_runtime_evidence_package import (
    build_lambda_l3_direct_tcp_runtime_evidence_package_from_dir,
    write_lambda_l3_direct_tcp_runtime_evidence_package,
)

API_BASE = "https://cloud.lambdalabs.com/api/v1"
REMOTE_SRC = "/home/ubuntu/diloco_l3_src"
REMOTE_RUN = "/home/ubuntu/lambda_l3_run"
DEFAULT_PORT = 28080
...
def main() -> int:
    ...
    parser.add_argument("--run-id", default=f"lambda-l3-{int(time.time())}")
    parser.add_argument(
        "--evidence-root",
        type=Path,
        default=Path("docs/evidence/lambda_l3_direct_tcp_adamw_nesterov"),
    )
    ...
    try:
        ...
        _run_learners(owned, syncer.ip, args)
    ...
    if owned:
        package = build_lambda_l3_direct_tcp_runtime_evidence_package_from_dir(evidence_dir)
        write_lambda_l3_direct_tcp_runtime_evidence_package(
            evidence_dir / "lambda_l3_evidence_package.json",
            package,
        )
        print(package.to_json(), end="")
        return 0 if package.lambda_l3_direct_tcp_runtime_passed else 2
    return 1
...
def _api(api_key: str, method: str, path: str, payload: dict[str, Any] | None = None) -> Any:
    ...
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "decodilo-lambda-l3-runner/0.1",
        },
    )
    try:
...
def _assert_no_live_instances(api_key: str) -> None:
    ...
    if instances:
        ...
        raise RuntimeError(f"refusing L3 launch while live instances exist: {ids}")
...
def _make_source_bundle(root: Path, run_id: str) -> Path:
    bundle = Path("/tmp") / f"diloco-l3-src-{run_id}.tar.gz"
...
def _install_source(ip: str, key: Path, bundle: Path) -> None:
    ...
    subprocess.run(
        [
            ...
            "UserKnownHostsFile=/dev/null",
            str(bundle),
            f"ubuntu@{ip}:/home/ubuntu/diloco_l3_src.tar.gz",
        ],
        check=True,
    )
    _ssh(
        ...
        key,
        (
            f"tar -xzf /home/ubuntu/diloco_l3_src.tar.gz -C {REMOTE_SRC} "
    ...
    *,
    ...
    for index, ip in enumerate(learner_ips):
        rule = {
            ...
            "description": f"Temporary Decodilo L3 learner-{index} direct TCP syncer access",
        }
...
def _restore_firewall_rules(
    ...
    *,
    ...
    try:
        ...
        restored = True
    except Exception as exc:  # noqa: BLE001 - record restore failure but keep terminating
        error = str(exc)
    evidence_dir.mkdir(parents=True, exist_ok=True)
...
async def main():
    async with JsonlTcpClient(host='127.0.0.1', port={args.port}, timeout_seconds=5) as client:
        envelope = make_envelope(
            ...
            payload={{'reason': 'lambda_l3_complete'}},
        )
