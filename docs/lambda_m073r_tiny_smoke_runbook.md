# M073R Tiny Decodilo Smoke Runbook Preview

M073R is a future supervised live Lambda milestone. It is not authorized by
this document alone.

Future M073R may run only after fresh read-only discovery, one-shot arming, and
...
env PYTHONPATH=/tmp/decodilo-runtime:/tmp/decodilo-src/src \
  python3 -m decodilo.cli dev tiny-smoke \
  --synthetic \
  --max-steps 1 \
  --out /tmp/decodilo-tiny-smoke.json
```
...
- exactly one tiny Decodilo smoke command
