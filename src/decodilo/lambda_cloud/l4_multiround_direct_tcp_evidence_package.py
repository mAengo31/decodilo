"""Offline evidence package for Lambda L4 multi-round direct TCP learner/syncer runtime."""
...
class LambdaL4DirectTcpRuntimeEvidencePackage(BaseModel):
    """Validated summary for the Lambda L4 remote multi-instance proof."""

    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "Lambda-L4"
    evidence_complete: bool
    lambda_l4_multiround_direct_tcp_runtime_passed: bool
    ...
    def _validate_boundaries(self) -> LambdaL4DirectTcpRuntimeEvidencePackage:
        if self.launch_ready or self.launch_allowed:
            raise ValueError("Lambda L4 evidence package cannot enable launch")
        if self.production_scale_ready:
            raise ValueError("Lambda L4 evidence package cannot claim production scale")
        if self.pathway_operation_layer_ready:
            raise ValueError("Lambda L4 evidence package cannot claim Pathway readiness")
...
def build_lambda_l4_multiround_direct_tcp_evidence_package_from_dir(
    evidence_dir: str | Path,
) -> LambdaL4DirectTcpRuntimeEvidencePackage:
    ...
    return LambdaL4DirectTcpRuntimeEvidencePackage(
        evidence_complete=evidence_complete,
        lambda_l4_multiround_direct_tcp_runtime_passed=evidence_complete,
        ...
        blockers=blockers,
        warnings=[
            "Lambda L4 proof uses real billable Lambda instances historically; parser is offline.",
            (
                "L4 proves direct TCP remote process/instance split, "
...
def load_lambda_l4_multiround_direct_tcp_evidence_package(
    path: str | Path,
) -> LambdaL4DirectTcpRuntimeEvidencePackage:
    return LambdaL4DirectTcpRuntimeEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_l4_multiround_direct_tcp_evidence_package(
    path: str | Path,
    package: LambdaL4DirectTcpRuntimeEvidencePackage,
) -> None:
    target = Path(path)
