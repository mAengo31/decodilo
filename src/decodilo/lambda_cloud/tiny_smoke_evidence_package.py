"""M073R2 remote Decodilo tiny-smoke evidence package."""
...
class LambdaTinySmokeEvidencePackage(BaseModel):
    ...
    def _validate_package(self) -> LambdaTinySmokeEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M074 evidence package must remain offline and disabled")
...
def build_lambda_tiny_smoke_evidence_package_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
) -> LambdaTinySmokeEvidencePackage:
    ...
    return LambdaTinySmokeEvidencePackage(
...
def load_lambda_tiny_smoke_evidence_package(path: str | Path) -> LambdaTinySmokeEvidencePackage:
    return LambdaTinySmokeEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_tiny_smoke_evidence_package(
    path: str | Path,
    report: LambdaTinySmokeEvidencePackage,
) -> None:
    target = Path(path)
