"""Offline audit of the M077R synthetic experiment artifact."""
...
class LambdaSyntheticExperimentArtifactAudit(BaseModel):
    ...
    def _validate_audit(self) -> LambdaSyntheticExperimentArtifactAudit:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("synthetic experiment artifact audit must remain offline")
...
def build_lambda_synthetic_experiment_artifact_audit_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
) -> LambdaSyntheticExperimentArtifactAudit:
    ...
    if record.artifact_path != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_PATH:
        blockers.append("unexpected_artifact_path")
    if record.artifact_bytes != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_BYTES:
        blockers.append("unexpected_artifact_size")
    if record.artifact_sha256 != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_SHA256:
    ...
    if record.synthetic_experiment_status != "passed":
        blockers.append("synthetic_experiment_status_not_passed")
    if record.learner_or_runtime_check_passed is not True:
    ...
    return LambdaSyntheticExperimentArtifactAudit(
        ...
        synthetic_experiment_status=record.synthetic_experiment_status,
        learner_or_runtime_check_passed=record.learner_or_runtime_check_passed,
        ...
        blockers=blockers,
        warnings=["artifact audit uses persisted M077R evidence only"],
    )


def load_lambda_synthetic_experiment_artifact_audit(
    path: str | Path,
) -> LambdaSyntheticExperimentArtifactAudit:
    return LambdaSyntheticExperimentArtifactAudit.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_synthetic_experiment_artifact_audit(
    path: str | Path,
    report: LambdaSyntheticExperimentArtifactAudit,
) -> None:
    target = Path(path)
