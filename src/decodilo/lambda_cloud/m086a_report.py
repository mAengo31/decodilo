"""Aggregate M086A parameter-fragment smoke command and M087R authorization."""

from __future__ import annotations

import json
from pathlib import Path

from pydantic import BaseModel, ConfigDict, Field, model_validator

from decodilo.lambda_cloud.m087r_parameter_fragment_authorization import (
    load_lambda_m087r_parameter_fragment_authorization,
)
from decodilo.lambda_cloud.m087r_parameter_fragment_runbook_preview import (
    load_lambda_m087r_parameter_fragment_runbook_preview,
)
from decodilo.lambda_cloud.parameter_fragment_command_discovery import (
    load_lambda_parameter_fragment_command_discovery,
)
from decodilo.lambda_cloud.parameter_fragment_policy import (
    load_lambda_parameter_fragment_policy,
)


class LambdaM086AReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M086A"
    report_passed: bool
    parameter_fragment_smoke_command_added: bool
    discovery_status: str
    selected_command: list[str] = Field(default_factory=list)
    policy_status: str
    m087r_authorization_status: str
    runbook_preview_status: str
    fragment_semantics_status: str | None = None
    fragments: int | None = None
    max_steps: int | None = None
    blockers: list[str] = Field(default_factory=list)
    warnings: list[str] = Field(default_factory=list)
    launch_ready: bool = False
    launch_allowed: bool = False
    billable_action_performed: bool = False

    @model_validator(mode="after")
    def _validate_report(self) -> LambdaM086AReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M086A report must not authorize launch or spend")
        if self.report_passed and self.blockers:
            raise ValueError("passing M086A report cannot carry blockers")
        return self

    def to_json(self) -> str:
        return json.dumps(self.model_dump(mode="json"), indent=2, sort_keys=True) + "\n"


def build_lambda_m086a_report_from_paths(
    *,
    command_discovery: str | Path,
    policy: str | Path,
    authorization: str | Path,
    runbook_preview: str | Path,
) -> LambdaM086AReport:
    discovery = load_lambda_parameter_fragment_command_discovery(command_discovery)
    fragment_policy = load_lambda_parameter_fragment_policy(policy)
    auth = load_lambda_m087r_parameter_fragment_authorization(authorization)
    preview = load_lambda_m087r_parameter_fragment_runbook_preview(runbook_preview)
    blockers: list[str] = []
    if discovery.discovery_status != "found_safe_parameter_fragment_command":
        blockers.append("parameter_fragment_smoke_command_not_found")
    if discovery.expected_parameter_fragment_semantics != "synthetic_vector_fragments":
        blockers.append("parameter_fragment_smoke_not_verified")
    if fragment_policy.policy_status != "policy_passed":
        blockers.append("parameter_fragment_policy_not_passed")
    if auth.authorization_status != "authorized_for_future_m087r_parameter_fragment_smoke":
        blockers.append("m087r_not_authorized")
    if preview.preview_status != "ready_for_future_m087r_parameter_fragment_review":
        blockers.append("m087r_runbook_preview_not_ready")
    return LambdaM086AReport(
        report_passed=not blockers,
        parameter_fragment_smoke_command_added=not blockers,
        discovery_status=discovery.discovery_status,
        selected_command=discovery.argv_tokens,
        policy_status=fragment_policy.policy_status,
        m087r_authorization_status=auth.authorization_status,
        runbook_preview_status=preview.preview_status,
        fragment_semantics_status=discovery.expected_parameter_fragment_semantics,
        fragments=discovery.fragments,
        max_steps=discovery.max_steps,
        blockers=blockers,
        warnings=["M086A is offline; M087R still requires fresh supervised approval"],
    )


def load_lambda_m086a_report(path: str | Path) -> LambdaM086AReport:
    return LambdaM086AReport.model_validate_json(Path(path).read_text(encoding="utf-8"))


def write_lambda_m086a_report(path: str | Path, report: LambdaM086AReport) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(report.to_json(), encoding="utf-8")
