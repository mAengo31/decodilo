"""M056 execution gate-check for the live-candidate SSH retry."""
...
class LambdaSSHConnectivityM056GateCheck(BaseModel):
    ...
    def _validate_disabled(self) -> LambdaSSHConnectivityM056GateCheck:
        if (
            self.launch_ready
            ...
            raise ValueError("M056 gate violates one-shot no-exec constraints")
        if self.gate_passed and self.blockers:
            raise ValueError("passing M056 gate cannot carry blockers")
        return self
...
def build_lambda_ssh_connectivity_m056_gate_check_from_paths(
    *,
    plan: str | Path,
...
) -> LambdaSSHConnectivityM056GateCheck:
    ...
    retry = load_lambda_ssh_retry_candidate_policy(retry_policy)
    ...
    if plan_report.plan_status != "plan_passed":
        blockers.extend(plan_report.blockers or ["m056_plan_not_passed"])
    if plan_report.selected_candidate != M056_SELECTED_CANDIDATE:
        blockers.append("m056_candidate_must_be_gpu_1x_a10")
    if plan_report.selected_region != M056_SELECTED_REGION:
        blockers.append("m056_region_must_be_us_east_1")
    if retry.policy_status != "policy_passed":
        blockers.extend(retry.blockers or ["m056_retry_policy_not_passed"])
    if not retry.no_automatic_retry:
        blockers.append("retry_policy_allows_automatic_retry")
    ...
    if audit.package_install_allowed or audit.training_allowed:
        blockers.append("audit_allows_install_or_training")
    ...
    return LambdaSSHConnectivityM056GateCheck(
        ...
        max_ssh_attempts=plan_report.max_ssh_connectivity_attempts,
        max_launch_attempts=plan_report.max_launch_attempts,
        no_auto_retry=retry.no_automatic_retry,
        no_auto_launch_retry=plan_report.no_auto_launch_retry,
        command_uses_no_shell_form=(
            "-N" in safe.command_preview and "-T" in safe.command_preview
        ),
        blockers=sorted(set(blockers)),
        warnings=[
            "M056 gate is a pre-launch artifact and does not SSH",
...
def load_lambda_ssh_connectivity_m056_gate_check(
    path: str | Path,
) -> LambdaSSHConnectivityM056GateCheck:
    return LambdaSSHConnectivityM056GateCheck.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_ssh_connectivity_m056_gate_check(
    path: str | Path,
    report: LambdaSSHConnectivityM056GateCheck,
) -> None:
    target = Path(path)
