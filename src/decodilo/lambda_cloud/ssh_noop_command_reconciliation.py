"""Offline reconciliation for the completed M057 SSH no-op command run."""
...
class LambdaSSHNoopCommandReconciliation(BaseModel):
    ...
    def _validate_closeout_only(self) -> LambdaSSHNoopCommandReconciliation:
        if (
            self.launch_ready
            ...
            raise ValueError("M058 reconciliation cannot enable launch or mutation")
        return self
...
def build_lambda_ssh_noop_command_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
    final_discovery: str | Path | None = None,
) -> LambdaSSHNoopCommandReconciliation:
    ...
    command_scope_respected = (
        ...
        and evidence.get("approved_command") == "true"
        and evidence.get("command_output_collected") is False
    ...
    return LambdaSSHNoopCommandReconciliation(
        ...
        warnings=["M058 reconciliation is offline and reads persisted M057 artifacts only"],
        errors=sorted(set(errors)),
    )


def load_lambda_ssh_noop_command_reconciliation(
    path: str | Path,
) -> LambdaSSHNoopCommandReconciliation:
    return LambdaSSHNoopCommandReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_ssh_noop_command_reconciliation(
    path: str | Path,
    report: LambdaSSHNoopCommandReconciliation,
) -> None:
    target = Path(path)
