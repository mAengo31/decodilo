"""M063 GPU visibility query planning and one-shot execution gates."""
...
M063_ARMED_FOR = "m063_gpu_visibility_query_single_launch_attempt"


class LambdaM063GPUVisibilityPlan(BaseModel):
    ...
    command_argv: list[str] = Field(default_factory=lambda: list(M063_COMMAND_ARGV))
    quantity: int = 1
    ...
    gpu_visibility_query_only: bool = True
    ...
    def _validate_disabled(self) -> LambdaM063GPUVisibilityPlan:
        if (
            self.launch_ready
            or self.launch_allowed
            or self.billable_action_performed
            or self.real_mutation_enabled
            or self.command != M063_GPU_VISIBILITY_COMMAND
            or self.command_argv != M063_COMMAND_ARGV
            or self.quantity != 1
            ...
            or not self.gpu_visibility_query_only
            ...
            raise ValueError("M063 GPU visibility plan violates constraints")
        if self.plan_status == "plan_passed" and self.blockers:
            raise ValueError("passing M063 plan cannot carry blockers")
        return self
...
class LambdaM063GPUVisibilityGateCheck(BaseModel):
    model_config = ConfigDict(frozen=True)
    ...
    @model_validator(mode="after")
    def _validate_gate(self) -> LambdaM063GPUVisibilityGateCheck:
        if (
            self.launch_ready
            or self.launch_allowed
            or self.billable_action_performed
            ...
            raise ValueError("M063 gate violates GPU query-only constraints")
        if self.gate_passed and self.blockers:
            raise ValueError("passing M063 gate cannot carry blockers")
        return self
...
class LambdaM063OneShotArming(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    arming_id: str
    arming_status: Literal["not_armed", "armed_for_one_shot_m063_gpu_visibility_query"]
    armed_for: str = M063_ARMED_FOR
    one_shot_request_send_permitted: bool = False
    request_send_permission_delegated_to_reviewer_bridge: bool = True
    selected_candidate: str | None = None
    selected_region: str | None = None
    approved_command: str = M063_GPU_VISIBILITY_COMMAND
    max_launch_attempts: int = 1
    max_ssh_connectivity_attempts: int = 1
    max_remote_command_attempts: int = 1
    no_auto_retry: bool = True
    ...
    @model_validator(mode="after")
    def _validate_arming(self) -> LambdaM063OneShotArming:
        if (
            self.armed_for != M063_ARMED_FOR
            or self.one_shot_request_send_permitted
            or self.launch_ready
            or self.launch_allowed
            or self.billable_action_performed
            or self.real_mutation_enabled
            or self.approved_command != M063_GPU_VISIBILITY_COMMAND
            or self.max_launch_attempts != 1
            or self.max_ssh_connectivity_attempts != 1
            or self.max_remote_command_attempts != 1
            or not self.no_auto_retry
            ...
            raise ValueError("M063 arming violates one-shot constraints")
        if self.arming_status == "armed_for_one_shot_m063_gpu_visibility_query" and self.blockers:
            raise ValueError("armed M063 artifact cannot have blockers")
        return self
...
class LambdaM063ReviewerBridge(BaseModel):
    model_config = ConfigDict(frozen=True)
    ...
    @model_validator(mode="after")
    def _validate_bridge(self) -> LambdaM063ReviewerBridge:
        if (
            self.launch_ready
            or self.launch_allowed
            or self.billable_action_performed
            ...
            raise ValueError("M063 reviewer bridge violates one-shot constraints")
        if self.bridge_status == "reviewer_compatible_one_shot_ready":
            if (
                ...
                raise ValueError("ready M063 bridge requires one-shot permissions")
        return self
...
def build_lambda_m063_gpu_visibility_plan_from_paths(
    *,
...
) -> LambdaM063GPUVisibilityPlan:
    return build_lambda_m063_gpu_visibility_plan(
...
def build_lambda_m063_gpu_visibility_plan(
    *,
    ...
    auth = load_lambda_m063_gpu_visibility_authorization(authorization_path)
    command_policy = load_lambda_gpu_visibility_command_policy(command_policy_path)
    output_policy = load_lambda_gpu_visibility_output_policy(output_policy_path)
    review = load_lambda_gpu_visibility_command_review(command_review_path)
    ...
    if (
        ...
        blockers.extend(auth.blockers or ["m063_authorization_not_ready"])
    if auth.selected_future_command_set != [M063_GPU_VISIBILITY_COMMAND]:
        blockers.append("m063_authorization_must_select_exact_gpu_visibility_command")
    if command_policy.command_policy_status != (
        "gpu_visibility_command_policy_defined_future_only"
    ):
        blockers.extend(command_policy.blockers or ["gpu_visibility_command_policy_failed"])
    if command_policy.selected_future_command_set != [M063_GPU_VISIBILITY_COMMAND]:
        blockers.append("gpu_visibility_command_policy_must_select_exact_command")
    if output_policy.output_policy_status != (
        "gpu_visibility_output_policy_defined_future_only"
    ):
        blockers.extend(output_policy.blockers or ["gpu_visibility_output_policy_failed"])
    if output_policy.max_stdout_bytes > 4096 or output_policy.max_stderr_bytes > 4096:
        blockers.append("gpu_visibility_output_capture_not_bounded")
    if review.command_review_status != "gpu_visibility_command_review_passed_future_only":
        blockers.extend(review.blockers or ["gpu_visibility_command_review_failed"])
    if review.selected_future_command_set != [M063_GPU_VISIBILITY_COMMAND]:
        blockers.append("gpu_visibility_command_review_must_select_exact_command")
    if not validate_gpu_visibility_command(M063_GPU_VISIBILITY_COMMAND):
        blockers.append("m063_gpu_visibility_command_invalid")
    ...
    if live_item is None:
        blockers.append("m063_selected_candidate_not_live_available_in_us_east_1")
    ...
    return LambdaM063GPUVisibilityPlan(
        ...
        warnings=[
            "M063 plan is non-executable until a one-shot reviewer bridge is used",
            "selected candidate must be freshly live-available before request construction",
            "only the exact bounded GPU visibility query is in scope",
            (
                "raw nvidia-smi, shell wrappers, Python, installs, benchmarks, "
                "and training remain forbidden"
            ),
        ],
    )


def build_lambda_m063_gpu_visibility_gate_check_from_paths(
    *,
    plan: str | Path,
    authorization: str | Path,
    command_policy: str | Path,
    output_policy: str | Path,
) -> LambdaM063GPUVisibilityGateCheck:
    plan_report = load_lambda_m063_gpu_visibility_plan(plan)
    ...
    if plan_report.plan_status != "plan_passed":
        blockers.append("m063_plan_not_passed")
    if (
        auth.authorization_status
        != "authorized_for_future_m063_gpu_visibility_query_review"
    ):
    ...
    if auth.selected_future_command_set != [M063_GPU_VISIBILITY_COMMAND]:
        blockers.append("m063_authorization_must_select_exact_gpu_visibility_command")
    if command.selected_future_command_set != [M063_GPU_VISIBILITY_COMMAND]:
        blockers.append("m063_command_policy_must_select_exact_gpu_visibility_command")
    if output.max_stdout_bytes > 4096 or output.max_stderr_bytes > 4096:
        blockers.append("m063_output_capture_not_bounded")
    if (
        plan_report.command != M063_GPU_VISIBILITY_COMMAND
        or plan_report.command_argv != M063_COMMAND_ARGV
    ):
        blockers.append("m063_plan_command_must_equal_exact_gpu_query")
    if plan_report.max_remote_command_attempts != 1:
        blockers.append("max_remote_commands_must_equal_one")
    return LambdaM063GPUVisibilityGateCheck(
        ...
        blockers=sorted(set(blockers)),
        warnings=[
            "M063 gate permits exactly one GPU visibility query command",
            (
                "raw nvidia-smi, shell wrappers, command chaining, transfer, install, "
                "benchmarks, Python, and training remain forbidden"
            ),
        ],
    )


def build_lambda_m063_one_shot_arming_from_paths(
    *,
    gate_check: str | Path,
    response_loss_controls: str | Path,
    expires_minutes: int,
    created_at_utc: str | None = None,
) -> LambdaM063OneShotArming:
    ...
    gate = load_lambda_m063_gpu_visibility_gate_check(gate_check)
    controls = load_lambda_strand_response_loss_control_check(response_loss_controls)
    blockers = list(gate.blockers)
    if not gate.gate_passed:
        blockers.append("m063_gate_not_passed")
    ...
    status: Literal["not_armed", "armed_for_one_shot_m063_gpu_visibility_query"] = (
        "armed_for_one_shot_m063_gpu_visibility_query" if not blockers else "not_armed"
    )
    arming_id = "m063-gpu-visibility-" + _hash_json(
        {
    ...
    return LambdaM063OneShotArming(
        arming_id=arming_id,
        ...
        blockers=sorted(set(blockers)),
        warnings=[
            "M063 arming is preview-only; reviewer bridge exposes one-shot permission",
            "standing launch_ready and launch_allowed remain false",
        ],
    )


def build_lambda_m063_reviewer_bridge_from_path(
    *,
    arming: str | Path,
    now_utc: str | None = None,
) -> LambdaM063ReviewerBridge:
    arming_report = load_lambda_m063_one_shot_arming(arming)
    blockers = list(arming_report.blockers)
    if arming_report.arming_status != "armed_for_one_shot_m063_gpu_visibility_query":
        blockers.append("m063_one_shot_arming_not_armed")
    if is_lambda_m063_one_shot_arming_expired(arming_report, now_utc=now_utc):
        blockers.append("m063_one_shot_arming_expired")
    ...
    return LambdaM063ReviewerBridge(
        ...
        blockers=sorted(set(blockers)),
        warnings=[
            "M063 bridge is the only artifact exposing one-shot GPU query permission",
            "standing launch_ready and launch_allowed remain false",
        ],
    )


def is_lambda_m063_one_shot_arming_expired(
    report: LambdaM063OneShotArming,
    *,
    now_utc: str | None = None,
) -> bool:
...
def load_lambda_m063_gpu_visibility_plan(
    path: str | Path,
) -> LambdaM063GPUVisibilityPlan:
    return LambdaM063GPUVisibilityPlan.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def load_lambda_m063_gpu_visibility_gate_check(
    path: str | Path,
) -> LambdaM063GPUVisibilityGateCheck:
    return LambdaM063GPUVisibilityGateCheck.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def load_lambda_m063_one_shot_arming(path: str | Path) -> LambdaM063OneShotArming:
    return LambdaM063OneShotArming.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def load_lambda_m063_reviewer_bridge(path: str | Path) -> LambdaM063ReviewerBridge:
    return LambdaM063ReviewerBridge.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m063_gpu_visibility_plan(
    path: str | Path,
    report: LambdaM063GPUVisibilityPlan,
) -> None:
    _write_json(path, report.to_json())


def write_lambda_m063_gpu_visibility_gate_check(
    path: str | Path,
    report: LambdaM063GPUVisibilityGateCheck,
) -> None:
    _write_json(path, report.to_json())


def write_lambda_m063_one_shot_arming(
    path: str | Path,
    report: LambdaM063OneShotArming,
) -> None:
    _write_json(path, report.to_json())
...
def write_lambda_m063_reviewer_bridge(
    path: str | Path,
    report: LambdaM063ReviewerBridge,
) -> None:
    _write_json(path, report.to_json())


def m063_plan_hash(plan: LambdaM063GPUVisibilityPlan) -> str:
    material = {
