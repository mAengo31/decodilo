"""Safe parser for learner/syncer smoke JSON artifacts."""
...
LearnerSyncerArtifactParseStatus = Literal[
    "parsed_safe_learner_syncer_smoke_artifact",
    "parsed_redacted_learner_syncer_smoke_artifact",
...
class LearnerSyncerArtifactParserReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M079S"
    parse_status: LearnerSyncerArtifactParseStatus
    artifact_path: str
    ...
    def _validate_report(self) -> LearnerSyncerArtifactParserReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("learner/syncer artifact parser must remain offline")
...
def _load_policy(
    policy: str | Path | dict[str, Any] | LambdaRemoteVSliceDeclaredArtifactPolicy,
) -> Any:
    if isinstance(policy, str | Path):
        return load_lambda_remote_vslice_declared_artifact_policy(policy)
    return policy


def parse_learner_syncer_artifact_file(
    *,
    artifact_path: str | Path,
    policy: str | Path | dict[str, Any] | LambdaRemoteVSliceDeclaredArtifactPolicy,
) -> LearnerSyncerArtifactParserReport:
    ...
    if _policy_value(loaded_policy, "policy_status", "policy_defined") != "policy_defined":
        return LearnerSyncerArtifactParserReport(
            parse_status="blocked_policy_not_defined",
            artifact_path=str(artifact_path),
            declared_artifact_path=declared_path,
            artifact_exists=False,
            max_content_bytes=max_bytes,
            blockers=["declared_artifact_policy_not_defined"],
        )
    ...
    if secret_scan_passed:
        return LearnerSyncerArtifactParserReport(
            parse_status="parsed_safe_learner_syncer_smoke_artifact",
            artifact_path=str(path),
            declared_artifact_path=declared_path,
            artifact_exists=True,
            artifact_bytes=size,
    ...
    return LearnerSyncerArtifactParserReport(
        parse_status="parsed_redacted_learner_syncer_smoke_artifact",
        artifact_path=str(path),
        declared_artifact_path=declared_path,
        artifact_exists=True,
        artifact_bytes=size,
...
def load_learner_syncer_artifact_parser_report(
    path: str | Path,
) -> LearnerSyncerArtifactParserReport:
    return LearnerSyncerArtifactParserReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_learner_syncer_artifact_parser_report(
    path: str | Path,
    report: LearnerSyncerArtifactParserReport,
) -> None:
    target = Path(path)
