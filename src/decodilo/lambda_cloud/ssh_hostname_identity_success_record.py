"""Success record for the completed M059 hostname identity command run."""
...
LambdaSSHHostnameIdentitySuccessStatus = Literal[
    "ssh_hostname_identity_success",
    "ssh_hostname_identity_partial",
    "ssh_hostname_identity_failed",
]


class LambdaSSHHostnameIdentitySuccessRecord(BaseModel):
    ...
    status: LambdaSSHHostnameIdentitySuccessStatus
    ...
    def _validate_closeout_only(self) -> LambdaSSHHostnameIdentitySuccessRecord:
        if (
            self.launch_ready
            ...
            raise ValueError("M060 success record cannot enable launch or mutation")
        if self.command != "hostname" or self.command_category != "identity_hostname":
            raise ValueError("M060 success record can only classify hostname")
        if self.status == "ssh_hostname_identity_success" and (
            self.stdout_stored
            ...
            raise ValueError("successful M059 hostname record includes forbidden work")
        return self
...
def build_lambda_ssh_hostname_identity_success_record_from_paths(
    *,
    workdir: str | Path,
    final_discovery: str | Path,
) -> LambdaSSHHostnameIdentitySuccessRecord:
    workdir_path = Path(workdir)
    discovery_path = Path(final_discovery)
    ...
    blockers: list[str] = []
    if report.run_id != "lambda-m059-hostname-identity-command":
        blockers.append("m059_run_id_not_confirmed")
    ...
    if report.remote_command != "hostname":
        blockers.append("remote_command_not_hostname")
    ...
    if evidence.get("approved_command") != "hostname":
        blockers.append("approved_command_not_hostname")
    if evidence.get("command_output_collected") is not True:
        blockers.append("hostname_stdout_not_collected")
    if evidence.get("stdout_redacted") != "<redacted-hostname>":
        blockers.append("hostname_stdout_not_redacted")
    if evidence.get("stdout_stored") is not False:
        blockers.append("stdout_stored")
    if not report.stdout_redacted_present or not report.stdout_secret_scan_passed:
        blockers.append("stdout_redaction_or_scan_missing")
    ...
    status: LambdaSSHHostnameIdentitySuccessStatus = (
        ...
        if not blockers
            ...
            else "ssh_hostname_identity_failed"
        )
    )
    return LambdaSSHHostnameIdentitySuccessRecord(
        ...
        stdout_captured_redacted=bool(report.stdout_redacted_present),
        ...
        warnings=[
            "M060 records historical M059 billable action only",
            "M059 is classified as hostname identity-command success",
        ],
    )
...
def load_lambda_ssh_hostname_identity_success_record(
    path: str | Path,
) -> LambdaSSHHostnameIdentitySuccessRecord:
    return LambdaSSHHostnameIdentitySuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_ssh_hostname_identity_success_record(
    path: str | Path,
    report: LambdaSSHHostnameIdentitySuccessRecord,
) -> None:
    target = Path(path)
