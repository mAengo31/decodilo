"""M083R DiLoCo optimizer-fidelity reconciliation."""
...
class LambdaDilocoOptimizerReconciliation(BaseModel):
    ...
    def _validate_reconciliation(self) -> LambdaDilocoOptimizerReconciliation:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M084 reconciliation must remain offline and disabled")
...
def build_lambda_diloco_optimizer_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
) -> LambdaDilocoOptimizerReconciliation:
    ...
    optimizer_semantics_confirmed = (
        record.optimization_fidelity == "optimizer_semantics_smoke"
    ...
    if not record.diloco_optimizer_smoke_command_passed:
        errors.append("diloco_optimizer_smoke_command_not_passed")
    ...
    if not optimizer_semantics_confirmed:
        errors.append("optimizer_semantics_not_confirmed")
    if not record.termination_verified:
        errors.append("termination_not_verified")
    return LambdaDilocoOptimizerReconciliation(
        ...
        diloco_optimizer_smoke_command_passed=record.diloco_optimizer_smoke_command_passed,
        artifact_metadata_confirmed=artifact_metadata_confirmed,
        artifact_body_confirmed=artifact_body_confirmed,
        optimizer_semantics_confirmed=optimizer_semantics_confirmed,
        termination_verified=record.termination_verified,
        warnings=["M084 reconciliation uses persisted M083R artifacts only"],
        errors=errors,
    )


def load_lambda_diloco_optimizer_reconciliation(
    path: str | Path,
) -> LambdaDilocoOptimizerReconciliation:
    return LambdaDilocoOptimizerReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_diloco_optimizer_reconciliation(
    path: str | Path,
    report: LambdaDilocoOptimizerReconciliation,
) -> None:
    target = Path(path)
