"""M079R2 learner/syncer smoke closeout."""
...
from decodilo.lambda_cloud.learner_syncer_smoke_evidence_package import (
    load_lambda_learner_syncer_smoke_evidence_package,
)
...
LambdaLearnerSyncerSmokeCloseoutStatus = Literal[
    "closed_success",
...
class LambdaLearnerSyncerSmokeCloseout(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M080"
    closeout_status: LambdaLearnerSyncerSmokeCloseoutStatus
    ...
    def _validate_closeout(self) -> LambdaLearnerSyncerSmokeCloseout:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M080 closeout must not enable launch or spend")
...
def build_lambda_learner_syncer_smoke_closeout_from_paths(
    *,
    ...
    package = load_lambda_learner_syncer_smoke_evidence_package(evidence_package)
    blockers: list[str] = []
    ...
    warnings = [
        "M080 is offline; historical M079R2 billable action remains recorded separately",
    ]
    if closeout_succeeded and (record.warnings or package.warnings):
        status: LambdaLearnerSyncerSmokeCloseoutStatus = "closed_with_warnings"
    elif closeout_succeeded:
        status = "closed_success"
    else:
        status = "unresolved"
    return LambdaLearnerSyncerSmokeCloseout(
...
def load_lambda_learner_syncer_smoke_closeout(
    path: str | Path,
) -> LambdaLearnerSyncerSmokeCloseout:
    return LambdaLearnerSyncerSmokeCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_learner_syncer_smoke_closeout(
    path: str | Path,
    report: LambdaLearnerSyncerSmokeCloseout,
) -> None:
    target = Path(path)
