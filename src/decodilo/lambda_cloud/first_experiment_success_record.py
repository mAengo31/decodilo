"""M071R first remote Decodilo experiment success record."""
...
M071R_REQUIRED_STAGES = (
    ...
    "first_experiment_command",
)
M071R_OUTPUT_ARTIFACT_PATH = "/tmp/decodilo-first-experiment-ci-profile-report.json"
M071R_OUTPUT_ARTIFACT_SHA256 = (
    "37324938d44f69a235233829d71fcf4d5b090226bbc5bfeb712ddcee73e152d3"
)
M071R_OUTPUT_ARTIFACT_BYTES = 3570
...
LambdaFirstExperimentSuccessStatus = Literal[
    "first_experiment_runtime_success",
    "first_experiment_runtime_partial",
    "first_experiment_runtime_failed",
]


class LambdaFirstExperimentSuccessRecord(BaseModel):
    model_config = ConfigDict(frozen=True)

    record_schema_version: int = 1
    milestone: str = "M071R"
    status: LambdaFirstExperimentSuccessStatus
    ...
    ci_profile_report_artifact_created: bool
    ...
    def _validate_offline_record(self) -> LambdaFirstExperimentSuccessRecord:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M072 success record must remain offline and disabled")
        if self.status == "first_experiment_runtime_success" and self.blockers:
            raise ValueError("successful first experiment record cannot carry blockers")
        return self
...
def build_lambda_first_experiment_success_record_from_paths(
    *,
    workdir: str | Path,
    ...
    if artifact_path != M071R_OUTPUT_ARTIFACT_PATH:
        blockers.append("unexpected_output_artifact_path")
    if artifact_sha != M071R_OUTPUT_ARTIFACT_SHA256:
        blockers.append("unexpected_output_artifact_sha256")
    if artifact_bytes != M071R_OUTPUT_ARTIFACT_BYTES:
        blockers.append("unexpected_output_artifact_size")
    ...
    if not blockers:
        ...
        status = "first_experiment_runtime_partial"
    else:
        status = "first_experiment_runtime_failed"
    ...
    return LambdaFirstExperimentSuccessRecord(
        ...
        first_experiment_command_passed=passed["first_experiment_command"],
        ci_profile_report_artifact_created=artifact_created,
        ...
        warnings=[
            "M072 records historical M071R billable work but performs no billable action",
        ],
    )


def load_lambda_first_experiment_success_record(
    path: str | Path,
) -> LambdaFirstExperimentSuccessRecord:
    return LambdaFirstExperimentSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_first_experiment_success_record(
    path: str | Path,
    record: LambdaFirstExperimentSuccessRecord,
) -> None:
    target = Path(path)
...
def _find_clean_post_summary(workdir: Path) -> Path | None:
    ...
    global_candidates = [
        Path("/tmp/decodilo-lambda-post-m071r-summary-final.json"),
        Path("/tmp/decodilo-lambda-post-m071r-summary-later.json"),
        Path("/tmp/decodilo-lambda-post-m071r-summary.json"),
    ]
...
def _existing_artifact_paths(workdir: Path, post_summary: Path | None) -> dict[str, str]:
    candidates = {
        ...
        "m070_report": Path("/tmp/decodilo-lambda-m070-report.json"),
        "m071r_manifest": Path("/tmp/decodilo-lambda-m071r-first-experiment-manifest.json"),
        "m071r_authorization": Path(
            "/tmp/decodilo-lambda-m071r-first-experiment-authorization.json"
        ),
        "m071r_runbook_preview": Path(
            "/tmp/decodilo-lambda-m071r-first-experiment-runbook-preview.json"
        ),
