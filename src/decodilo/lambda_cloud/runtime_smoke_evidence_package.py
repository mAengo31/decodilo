"""M075R4 remote Decodilo runtime/protocol smoke evidence package."""
...
class LambdaRuntimeSmokeEvidencePackage(BaseModel):
    ...
    def _validate_package(self) -> LambdaRuntimeSmokeEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M076 evidence package must remain offline and disabled")
...
def build_lambda_runtime_smoke_evidence_package_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
) -> LambdaRuntimeSmokeEvidencePackage:
    ...
    return LambdaRuntimeSmokeEvidencePackage(
...
def load_lambda_runtime_smoke_evidence_package(
    path: str | Path,
) -> LambdaRuntimeSmokeEvidencePackage:
    return LambdaRuntimeSmokeEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_runtime_smoke_evidence_package(
    path: str | Path,
    report: LambdaRuntimeSmokeEvidencePackage,
) -> None:
    target = Path(path)
