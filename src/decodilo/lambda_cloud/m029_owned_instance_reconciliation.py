"""Owned-instance reconciliation for lost M029 launch responses."""
...
LambdaM029OwnershipConfidence = Literal["exact", "high", "medium", "low", "none"]


class LambdaM029OwnedInstanceReconciliationReport(BaseModel):
    ...
    confidence: LambdaM029OwnershipConfidence = "none"
...
def reconcile_m029_owned_instance(
    *,
    ...
    if exact_owned:
        return LambdaM029OwnedInstanceReconciliationReport(
...
def reconcile_m029_owned_instance_from_paths(
    *,
    ...
    from decodilo.lambda_cloud.m029_discovery_diff import load_lambda_m029_discovery_diff

    return reconcile_m029_owned_instance(
        discovery_diff=load_lambda_m029_discovery_diff(discovery_diff),
...
def load_lambda_m029_owned_instance_reconciliation(
    path: str | Path,
) -> LambdaM029OwnedInstanceReconciliationReport:
    return LambdaM029OwnedInstanceReconciliationReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m029_owned_instance_reconciliation(
    path: str | Path,
    report: LambdaM029OwnedInstanceReconciliationReport,
) -> None:
    target = Path(path)
