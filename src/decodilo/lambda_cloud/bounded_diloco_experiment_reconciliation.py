"""M089R bounded synthetic DiLoCo experiment reconciliation."""
...
class LambdaBoundedDilocoExperimentReconciliation(BaseModel):
    ...
    def _validate_reconciliation(self) -> LambdaBoundedDilocoExperimentReconciliation:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M090 reconciliation must remain offline and disabled")
...
def build_lambda_bounded_diloco_experiment_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
) -> LambdaBoundedDilocoExperimentReconciliation:
    ...
    bounded_semantics_confirmed = (
        record.bounded_diloco_experiment_status == "passed"
        and record.optimization_fidelity == "bounded_synthetic_diloco_experiment"
        ...
        and record.learners_observed == 1
        and record.sync_rounds_completed == 1
        and record.fragments_observed == 2
        and record.learner_syncer_exchange_check_passed is True
        and record.update_or_commit_check_passed is True
        ...
        and record.integrated_reference_check_passed is True
        and record.max_abs_error == 0.0
        and record.full_diloco_training_claimed is False
        and record.real_model_training_claimed is False
        and record.true_model_fragment_claimed is False
    ...
    if not record.bounded_diloco_experiment_command_passed:
        errors.append("bounded_diloco_experiment_command_not_passed")
    ...
    if not bounded_semantics_confirmed:
        errors.append("bounded_experiment_semantics_not_confirmed")
    if not record.termination_verified:
        errors.append("termination_not_verified")
    return LambdaBoundedDilocoExperimentReconciliation(
        ...
        bounded_diloco_experiment_command_passed=(
            record.bounded_diloco_experiment_command_passed
        ),
        artifact_metadata_confirmed=artifact_metadata_confirmed,
        artifact_body_confirmed=artifact_body_confirmed,
        bounded_experiment_semantics_confirmed=bounded_semantics_confirmed,
        termination_verified=record.termination_verified,
        warnings=["M090 reconciliation uses persisted M089R artifacts only"],
        errors=errors,
    )


def load_lambda_bounded_diloco_experiment_reconciliation(
    path: str | Path,
) -> LambdaBoundedDilocoExperimentReconciliation:
    return LambdaBoundedDilocoExperimentReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_bounded_diloco_experiment_reconciliation(
    path: str | Path,
    report: LambdaBoundedDilocoExperimentReconciliation,
) -> None:
    target = Path(path)
