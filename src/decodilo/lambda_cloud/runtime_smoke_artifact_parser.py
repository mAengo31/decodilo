"""Safe parser for declared runtime-smoke JSON artifacts."""
...
RUNTIME_SMOKE_DECLARED_ARTIFACT_PATH = "/tmp/decodilo-runtime-smoke.json"
DEFAULT_MAX_CONTENT_BYTES = 32768
...
RuntimeSmokeArtifactParseStatus = Literal[
    "parsed_safe_runtime_smoke_artifact",
    "parsed_redacted_runtime_smoke_artifact",
...
class RuntimeSmokeArtifactParserReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M075T"
    parse_status: RuntimeSmokeArtifactParseStatus
    artifact_path: str
    ...
    def _validate_report(self) -> RuntimeSmokeArtifactParserReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("runtime-smoke artifact parser must remain offline")
        if self.raw_content_persisted and not self.raw_content_persist_allowed:
            raise ValueError("raw content cannot be persisted unless explicitly allowed")
        return self
...
def _summary_from_body(body: dict[str, Any], redactions: list[str]) -> dict[str, Any]:
    allowed_keys = [
        ...
        "learner_or_runtime_check_passed",
...
def parse_runtime_smoke_artifact_file(
    *,
    artifact_path: str | Path,
    policy: Any,
) -> RuntimeSmokeArtifactParserReport:
    path = Path(artifact_path)
    declared_path = str(
        _policy_value(policy, "declared_artifact_path", RUNTIME_SMOKE_DECLARED_ARTIFACT_PATH)
    )
    max_bytes = int(_policy_value(policy, "max_content_bytes", DEFAULT_MAX_CONTENT_BYTES))
    if not path.exists():
        return RuntimeSmokeArtifactParserReport(
    ...
    if secret_scan_passed:
        return RuntimeSmokeArtifactParserReport(
            parse_status="parsed_safe_runtime_smoke_artifact",
            artifact_path=str(path),
            declared_artifact_path=declared_path,
            artifact_exists=True,
            artifact_bytes=size,
    ...
    return RuntimeSmokeArtifactParserReport(
        parse_status="parsed_redacted_runtime_smoke_artifact",
        artifact_path=str(path),
        declared_artifact_path=declared_path,
        artifact_exists=True,
        artifact_bytes=size,
...
def load_runtime_smoke_artifact_parser_report(
    path: str | Path,
) -> RuntimeSmokeArtifactParserReport:
    return RuntimeSmokeArtifactParserReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_runtime_smoke_artifact_parser_report(
    path: str | Path,
    report: RuntimeSmokeArtifactParserReport,
) -> None:
    target = Path(path)
