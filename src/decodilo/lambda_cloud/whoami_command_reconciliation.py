"""Offline reconciliation for the completed M061 whoami identity command run."""
...
class LambdaWhoamiCommandReconciliation(BaseModel):
    ...
    def _validate_closeout_only(self) -> LambdaWhoamiCommandReconciliation:
        if (
            self.launch_ready
            ...
            raise ValueError("M062 reconciliation cannot enable launch or mutation")
        return self
...
def build_lambda_whoami_command_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
    final_discovery: str | Path | None = None,
) -> LambdaWhoamiCommandReconciliation:
    ...
    whoami_only = (
        ...
        and report.remote_command == "whoami"
        and evidence.get("approved_command") == "whoami"
    )
    ...
    if not whoami_only:
        errors.append("whoami_only_not_confirmed")
    ...
    return LambdaWhoamiCommandReconciliation(
        ...
        whoami_only_confirmed=whoami_only,
        no_other_command_confirmed=whoami_only,
        ...
        warnings=["M062 reconciliation is offline and reads persisted M061 artifacts only"],
        errors=sorted(set(errors)),
    )


def load_lambda_whoami_command_reconciliation(
    path: str | Path,
) -> LambdaWhoamiCommandReconciliation:
    return LambdaWhoamiCommandReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_whoami_command_reconciliation(
    path: str | Path,
    report: LambdaWhoamiCommandReconciliation,
) -> None:
    target = Path(path)
