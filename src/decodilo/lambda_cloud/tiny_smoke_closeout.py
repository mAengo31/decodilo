"""M073R2 remote Decodilo tiny-smoke closeout."""
...
from decodilo.lambda_cloud.tiny_smoke_evidence_package import (
    load_lambda_tiny_smoke_evidence_package,
)
...
LambdaTinySmokeCloseoutStatus = Literal[
    "closed_success",
...
class LambdaTinySmokeCloseout(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M074"
    closeout_status: LambdaTinySmokeCloseoutStatus
    closeout_succeeded: bool
    tiny_smoke_success: bool
    ...
    def _validate_closeout(self) -> LambdaTinySmokeCloseout:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M074 closeout must not enable launch or spend")
...
def build_lambda_tiny_smoke_closeout_from_paths(
    *,
    ...
    package = load_lambda_tiny_smoke_evidence_package(evidence_package)
    blockers: list[str] = []
    ...
    if not record.tiny_smoke_artifact_created:
    ...
    warnings = [
        "M074 is offline; historical M073R2 billable action remains recorded separately",
    ]
    status: LambdaTinySmokeCloseoutStatus
...
def load_lambda_tiny_smoke_closeout(path: str | Path) -> LambdaTinySmokeCloseout:
    return LambdaTinySmokeCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_tiny_smoke_closeout(
    path: str | Path,
    report: LambdaTinySmokeCloseout,
) -> None:
    target = Path(path)
