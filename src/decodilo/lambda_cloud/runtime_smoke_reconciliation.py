"""M075R4 remote Decodilo runtime/protocol smoke reconciliation."""
...
class LambdaRuntimeSmokeReconciliation(BaseModel):
    ...
    def _validate_reconciliation(self) -> LambdaRuntimeSmokeReconciliation:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M076 reconciliation must remain offline and disabled")
...
def build_lambda_runtime_smoke_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
) -> LambdaRuntimeSmokeReconciliation:
    ...
    if not record.runtime_smoke_command_passed:
        errors.append("runtime_smoke_command_not_passed")
    ...
    return LambdaRuntimeSmokeReconciliation(
        ...
        runtime_smoke_command_passed=record.runtime_smoke_command_passed,
        ...
        warnings=["M076 reconciliation uses persisted M075R4 artifacts only"],
        errors=errors,
    )


def load_lambda_runtime_smoke_reconciliation(
    path: str | Path,
) -> LambdaRuntimeSmokeReconciliation:
    return LambdaRuntimeSmokeReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_runtime_smoke_reconciliation(
    path: str | Path,
    report: LambdaRuntimeSmokeReconciliation,
) -> None:
    target = Path(path)
