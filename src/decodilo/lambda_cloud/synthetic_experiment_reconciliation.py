"""M077R remote bounded synthetic experiment reconciliation."""
...
class LambdaSyntheticExperimentReconciliation(BaseModel):
    ...
    def _validate_reconciliation(self) -> LambdaSyntheticExperimentReconciliation:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M078 reconciliation must remain offline and disabled")
...
def build_lambda_synthetic_experiment_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
) -> LambdaSyntheticExperimentReconciliation:
    ...
    if not record.synthetic_experiment_command_passed:
        errors.append("synthetic_experiment_command_not_passed")
    ...
    return LambdaSyntheticExperimentReconciliation(
        ...
        synthetic_experiment_command_passed=record.synthetic_experiment_command_passed,
        ...
        warnings=["M078 reconciliation uses persisted M077R artifacts only"],
        errors=errors,
    )


def load_lambda_synthetic_experiment_reconciliation(
    path: str | Path,
) -> LambdaSyntheticExperimentReconciliation:
    return LambdaSyntheticExperimentReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_synthetic_experiment_reconciliation(
    path: str | Path,
    report: LambdaSyntheticExperimentReconciliation,
) -> None:
    target = Path(path)
