"""Safe parser for integrated DiLoCo synthetic smoke JSON artifacts."""
...
IntegratedDilocoArtifactParseStatus = Literal[
    "parsed_safe_integrated_diloco_smoke_artifact",
    "parsed_redacted_integrated_diloco_smoke_artifact",
...
class IntegratedDilocoArtifactParserReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M085R"
    parse_status: IntegratedDilocoArtifactParseStatus
    artifact_path: str
    declared_artifact_path: str = INTEGRATED_DILOCO_SMOKE_DECLARED_ARTIFACT_PATH
    ...
    def _validate_report(self) -> IntegratedDilocoArtifactParserReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("integrated DiLoCo artifact parser must remain offline")
        ...
        if (
            ...
            raise ValueError("integrated smoke cannot claim parameter fragments")
        return self
...
def _summary_from_body(body: dict[str, Any], redactions: list[str]) -> dict[str, Any]:
    allowed_keys = [
        "integrated_diloco_smoke_status",
        ...
        "max_steps",
        "learner_count_observed",
        "syncer_role_check_passed",
...
def parse_integrated_diloco_artifact_file(
    *,
    artifact_path: str | Path,
    policy: str | Path | dict[str, Any] | Any,
) -> IntegratedDilocoArtifactParserReport:
    ...
    if _policy_value(loaded_policy, "policy_status", "policy_defined") not in {
        ...
        return IntegratedDilocoArtifactParserReport(
    ...
    if secret_scan_passed:
        return IntegratedDilocoArtifactParserReport(
            parse_status="parsed_safe_integrated_diloco_smoke_artifact",
            artifact_path=str(path),
            declared_artifact_path=declared_path,
            artifact_exists=True,
            artifact_bytes=size,
    ...
    return IntegratedDilocoArtifactParserReport(
        parse_status="parsed_redacted_integrated_diloco_smoke_artifact",
        artifact_path=str(path),
        declared_artifact_path=declared_path,
        artifact_exists=True,
        artifact_bytes=size,
...
def load_integrated_diloco_artifact_parser_report(
    path: str | Path,
) -> IntegratedDilocoArtifactParserReport:
    return IntegratedDilocoArtifactParserReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_integrated_diloco_artifact_parser_report(
    path: str | Path,
    report: IntegratedDilocoArtifactParserReport,
) -> None:
    target = Path(path)
