"""Read-only discovery diffing for M031 incident closeout."""
...
LambdaM031DiscoveryDiffConfidence = Literal[
...
class LambdaM031DiscoveryDiffReport(BaseModel):
    ...
    confidence: LambdaM031DiscoveryDiffConfidence
...
def build_lambda_m031_discovery_diff(
    *,
    ...
    if not pre_instances and not post_instances and not closeout_instances:
        confidence: LambdaM031DiscoveryDiffConfidence = "high_no_instance_created"
    elif candidates:
    ...
    return LambdaM031DiscoveryDiffReport(
...
def build_lambda_m031_discovery_diff_from_paths(
    *,
    ...
    return build_lambda_m031_discovery_diff(
...
def load_lambda_m031_discovery_diff(path: str | Path) -> LambdaM031DiscoveryDiffReport:
    return LambdaM031DiscoveryDiffReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m031_discovery_diff(
    path: str | Path,
    report: LambdaM031DiscoveryDiffReport,
) -> None:
    target = Path(path)
