"""Offline reconciliation for the completed M063 GPU visibility query run."""
...
class LambdaGPUVisibilityReconciliation(BaseModel):
    ...
    gpu_visibility_query_only_confirmed: bool
    no_other_command_confirmed: bool
    ...
    def _validate_closeout_only(self) -> LambdaGPUVisibilityReconciliation:
        if (
            self.launch_ready
            ...
            raise ValueError("M064 reconciliation cannot enable launch or mutation")
        return self
...
def build_lambda_gpu_visibility_reconciliation_from_paths(
    *,
    workdir: str | Path,
    success_record: str | Path,
    final_discovery: str | Path | None = None,
) -> LambdaGPUVisibilityReconciliation:
    ...
    query_only = (
        report.remote_command_attempted
        and report.remote_command == M063_GPU_VISIBILITY_COMMAND
        and evidence.get("approved_command") == M063_GPU_VISIBILITY_COMMAND
    )
    ...
    return LambdaGPUVisibilityReconciliation(
        ...
        gpu_visibility_query_only_confirmed=query_only,
        no_other_command_confirmed=query_only,
        ...
        warnings=["M064 reconciliation is offline and reads persisted M063 artifacts only"],
        errors=sorted(set(errors)),
    )


def load_lambda_gpu_visibility_reconciliation(
    path: str | Path,
) -> LambdaGPUVisibilityReconciliation:
    return LambdaGPUVisibilityReconciliation.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_gpu_visibility_reconciliation(
    path: str | Path,
    report: LambdaGPUVisibilityReconciliation,
) -> None:
    target = Path(path)
