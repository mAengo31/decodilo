"""M079R learner/syncer smoke attempt closeout."""
...
LEARNER_SYNCER_DECLARED_ARTIFACT_PATH = "/tmp/decodilo-learner-syncer-smoke.json"


class LambdaLearnerSyncerSmokeAttemptCloseout(BaseModel):
    ...
    learner_syncer_smoke_attempted: bool
    learner_syncer_smoke_exit_code: int | None = None
    learner_syncer_smoke_command_passed: bool
    expected_artifact_path: str = LEARNER_SYNCER_DECLARED_ARTIFACT_PATH
    ...
    def _validate_closeout(self) -> LambdaLearnerSyncerSmokeAttemptCloseout:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M079S closeout must remain offline")
        if self.closeout_succeeded and self.blockers:
            raise ValueError("successful M079S closeout cannot carry blockers")
        return self
...
def build_lambda_learner_syncer_smoke_attempt_closeout_from_paths(
    *,
    workdir: str | Path,
) -> LambdaLearnerSyncerSmokeAttemptCloseout:
    ...
    smoke_stage = _stage(report, "learner_syncer_smoke_command")
    ...
    if report.get("experiment_output_artifact_path") != LEARNER_SYNCER_DECLARED_ARTIFACT_PATH:
    ...
    return LambdaLearnerSyncerSmokeAttemptCloseout(
        closeout_status=(
        ...
        learner_syncer_smoke_attempted=bool(smoke_stage),
        learner_syncer_smoke_exit_code=smoke_stage.get("exit_code"),
        learner_syncer_smoke_command_passed=(
        ...
        warnings=[
            "M079R command path passed; closeout is for artifact capture policy only",
        ],
    )


def load_lambda_learner_syncer_smoke_attempt_closeout(
    path: str | Path,
) -> LambdaLearnerSyncerSmokeAttemptCloseout:
    return LambdaLearnerSyncerSmokeAttemptCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_learner_syncer_smoke_attempt_closeout(
    path: str | Path,
    closeout: LambdaLearnerSyncerSmokeAttemptCloseout,
) -> None:
    target = Path(path)
