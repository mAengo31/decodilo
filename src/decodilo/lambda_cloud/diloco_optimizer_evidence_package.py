"""M083R DiLoCo optimizer-fidelity evidence package."""
...
class LambdaDilocoOptimizerEvidencePackage(BaseModel):
    ...
    def _validate_package(self) -> LambdaDilocoOptimizerEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M084 evidence package must remain offline and disabled")
...
def build_lambda_diloco_optimizer_evidence_package_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
) -> LambdaDilocoOptimizerEvidencePackage:
    ...
    return LambdaDilocoOptimizerEvidencePackage(
...
def load_lambda_diloco_optimizer_evidence_package(
    path: str | Path,
) -> LambdaDilocoOptimizerEvidencePackage:
    return LambdaDilocoOptimizerEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_diloco_optimizer_evidence_package(
    path: str | Path,
    report: LambdaDilocoOptimizerEvidencePackage,
) -> None:
    target = Path(path)
