"""Owned-instance reconciliation for lost M031 launch responses."""
...
LambdaM031OwnershipConfidence = Literal["exact", "high", "medium", "low", "none"]


class LambdaM031OwnedInstanceReconciliationReport(BaseModel):
    ...
    confidence: LambdaM031OwnershipConfidence = "none"
...
def reconcile_m031_owned_instance(
    *,
    ...
    if exact_owned:
        return LambdaM031OwnedInstanceReconciliationReport(
...
def reconcile_m031_owned_instance_from_paths(
    *,
    ...
    from decodilo.lambda_cloud.m031_discovery_diff import load_lambda_m031_discovery_diff

    return reconcile_m031_owned_instance(
        discovery_diff=load_lambda_m031_discovery_diff(discovery_diff),
...
def load_lambda_m031_owned_instance_reconciliation(
    path: str | Path,
) -> LambdaM031OwnedInstanceReconciliationReport:
    return LambdaM031OwnedInstanceReconciliationReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m031_owned_instance_reconciliation(
    path: str | Path,
    report: LambdaM031OwnedInstanceReconciliationReport,
) -> None:
    target = Path(path)
