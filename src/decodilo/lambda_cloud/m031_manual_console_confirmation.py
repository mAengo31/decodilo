"""Manual Lambda console confirmation records for M031 incidents."""
...
LambdaM031ConsoleConfirmationStatus = Literal[
    "not_provided",
...
class LambdaM031ManualConsoleConfirmation(BaseModel):
    model_config = ConfigDict(frozen=True)

    confirmation_id: str = "lambda-m031-manual-console-confirmation"
    ...
    notes: str | None = None
    confirmation_status: LambdaM031ConsoleConfirmationStatus = "not_provided"
    launch_ready: bool = False
    launch_allowed: bool = False

    @model_validator(mode="after")
    def _validate_disabled_and_manual_termination(self) -> LambdaM031ManualConsoleConfirmation:
        if self.launch_ready or self.launch_allowed:
            raise ValueError("M031 manual console confirmation cannot enable launch")
        if (
...
class LambdaM031ManualConsoleConfirmationReport(LambdaM031ManualConsoleConfirmation):
    pass


def build_lambda_m031_manual_console_confirmation(
    *,
    lambda_console_checked: bool = False,
    no_instances_visible: bool = False,
    no_pending_instances_visible: bool = False,
    ...
    if not lambda_console_checked:
        status: LambdaM031ConsoleConfirmationStatus = "not_provided"
        ...
        status = "unresolved"
    return LambdaM031ManualConsoleConfirmationReport(
...
def load_lambda_m031_manual_console_confirmation(
    path: str | Path,
) -> LambdaM031ManualConsoleConfirmationReport:
    return LambdaM031ManualConsoleConfirmationReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m031_manual_console_confirmation(
    path: str | Path,
    report: LambdaM031ManualConsoleConfirmationReport,
) -> None:
    target = Path(path)
