"""M077R remote bounded synthetic experiment closeout."""
...
from decodilo.lambda_cloud.synthetic_experiment_evidence_package import (
    load_lambda_synthetic_experiment_evidence_package,
)
...
LambdaSyntheticExperimentCloseoutStatus = Literal[
    "closed_success",
...
class LambdaSyntheticExperimentCloseout(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M078"
    closeout_status: LambdaSyntheticExperimentCloseoutStatus
    ...
    def _validate_closeout(self) -> LambdaSyntheticExperimentCloseout:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M078 closeout must not enable launch or spend")
...
def build_lambda_synthetic_experiment_closeout_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
    evidence_package: str | Path,
) -> LambdaSyntheticExperimentCloseout:
    ...
    package = load_lambda_synthetic_experiment_evidence_package(evidence_package)
    blockers: list[str] = []
    ...
    warnings = [
        "M078 is offline; historical M077R billable action remains recorded separately",
    ]
    if closeout_succeeded and (record.warnings or package.warnings):
        status: LambdaSyntheticExperimentCloseoutStatus = "closed_with_warnings"
    elif closeout_succeeded:
        status = "closed_success"
    else:
        status = "unresolved"
    return LambdaSyntheticExperimentCloseout(
...
def load_lambda_synthetic_experiment_closeout(
    path: str | Path,
) -> LambdaSyntheticExperimentCloseout:
    return LambdaSyntheticExperimentCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_synthetic_experiment_closeout(
    path: str | Path,
    report: LambdaSyntheticExperimentCloseout,
) -> None:
    target = Path(path)
