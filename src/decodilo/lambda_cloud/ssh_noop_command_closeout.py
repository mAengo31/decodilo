"""Closeout for the successful M057 SSH no-op command run."""
...
from decodilo.lambda_cloud.ssh_noop_command_evidence_package import (
    load_lambda_ssh_noop_command_evidence_package,
)
...
LambdaSSHNoopCommandCloseoutStatus = Literal[
    "closed_success",
...
class LambdaSSHNoopCommandCloseout(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    closeout_status: LambdaSSHNoopCommandCloseoutStatus
    ...
    def _validate_closeout_only(self) -> LambdaSSHNoopCommandCloseout:
        if (
            self.launch_ready
            ...
            raise ValueError("M058 closeout cannot enable launch or mutation")
        if self.command != "true" or self.command_category != "noop":
            raise ValueError("M058 closeout can only close the true no-op command")
        return self
...
def build_lambda_ssh_noop_command_closeout_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
    evidence_package: str | Path,
) -> LambdaSSHNoopCommandCloseout:
    ...
    evidence = load_lambda_ssh_noop_command_evidence_package(evidence_package)
    ...
    if blockers:
        closeout_status: LambdaSSHNoopCommandCloseoutStatus = "unresolved"
    ...
    return LambdaSSHNoopCommandCloseout(
        ...
        warnings=sorted(
            set(
                [
                    "M058 closeout is offline and performs no Lambda operation",
                    *success.warnings,
                    *reconcile.warnings,
                    *evidence.warnings,
                ]
...
def load_lambda_ssh_noop_command_closeout(
    path: str | Path,
) -> LambdaSSHNoopCommandCloseout:
    return LambdaSSHNoopCommandCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_ssh_noop_command_closeout(
    path: str | Path,
    report: LambdaSSHNoopCommandCloseout,
) -> None:
    target = Path(path)
