"""M085R integrated synthetic DiLoCo evidence package."""
...
class LambdaIntegratedDilocoEvidencePackage(BaseModel):
    ...
    def _validate_package(self) -> LambdaIntegratedDilocoEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M086 evidence package must remain offline and disabled")
...
def build_lambda_integrated_diloco_evidence_package_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
) -> LambdaIntegratedDilocoEvidencePackage:
    ...
    return LambdaIntegratedDilocoEvidencePackage(
...
def load_lambda_integrated_diloco_evidence_package(
    path: str | Path,
) -> LambdaIntegratedDilocoEvidencePackage:
    return LambdaIntegratedDilocoEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_integrated_diloco_evidence_package(
    path: str | Path,
    report: LambdaIntegratedDilocoEvidencePackage,
) -> None:
    target = Path(path)
