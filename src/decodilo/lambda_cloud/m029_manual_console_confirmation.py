"""Manual Lambda console confirmation records for M029 incidents."""
...
LambdaM029ConsoleConfirmationStatus = Literal[
    "not_provided",
...
class LambdaM029ManualConsoleConfirmation(BaseModel):
    model_config = ConfigDict(frozen=True)

    confirmation_id: str = "lambda-m029-manual-console-confirmation"
    ...
    notes: str | None = None
    confirmation_status: LambdaM029ConsoleConfirmationStatus = "not_provided"
    launch_ready: bool = False
    launch_allowed: bool = False

    @model_validator(mode="after")
    def _validate_disabled_and_manual_termination(self) -> LambdaM029ManualConsoleConfirmation:
        if self.launch_ready or self.launch_allowed:
            raise ValueError("manual console confirmation cannot enable launch")
        if (
...
class LambdaM029ManualConsoleConfirmationReport(LambdaM029ManualConsoleConfirmation):
    pass


def build_lambda_m029_manual_console_confirmation(
    *,
    lambda_console_checked: bool = False,
    no_instances_visible: bool = False,
    no_pending_instances_visible: bool = False,
    ...
    if not lambda_console_checked:
        status: LambdaM029ConsoleConfirmationStatus = "not_provided"
        ...
        status = "unresolved"
    return LambdaM029ManualConsoleConfirmationReport(
...
def load_lambda_m029_manual_console_confirmation(
    path: str | Path,
) -> LambdaM029ManualConsoleConfirmationReport:
    return LambdaM029ManualConsoleConfirmationReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_m029_manual_console_confirmation(
    path: str | Path,
    report: LambdaM029ManualConsoleConfirmationReport,
) -> None:
    target = Path(path)
