"""M085R remote integrated synthetic DiLoCo success record."""
...
M085R_INTEGRATED_ARTIFACT_PATH = "/tmp/decodilo-integrated-diloco-smoke.json"
M085R_INTEGRATED_ARTIFACT_SHA256 = (
    "43902414b1ba7cb1a4ec15885d89216bc0d1b119b126e3d8aa982a7e190749f1"
)
M085R_INTEGRATED_ARTIFACT_BYTES = 5687
M085R_SOURCE_BUNDLE_SHA256 = (
    "83e2afdd12c12cfa6b3d6080dd668deea899212964aebe40fa78479ee74d59c5"
)
...
LambdaIntegratedDilocoSuccessStatus = Literal[
    "remote_integrated_diloco_synthetic_success",
    "remote_integrated_diloco_synthetic_partial",
    "remote_integrated_diloco_synthetic_failed",
]


class LambdaIntegratedDilocoSuccessRecord(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M086"
    success_status: LambdaIntegratedDilocoSuccessStatus
    ...
    def _validate_offline_record(self) -> LambdaIntegratedDilocoSuccessRecord:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M086 success record must remain offline and disabled")
        if (
            self.success_status == "remote_integrated_diloco_synthetic_success"
            and self.blockers
        ):
            raise ValueError("successful integrated record cannot carry blockers")
        return self
...
def build_lambda_integrated_diloco_success_record_from_paths(
    *,
    workdir: str | Path,
) -> LambdaIntegratedDilocoSuccessRecord:
    ...
    if artifact_path != M085R_INTEGRATED_ARTIFACT_PATH:
        blockers.append("unexpected_integrated_artifact_path")
    if artifact_sha != M085R_INTEGRATED_ARTIFACT_SHA256:
        blockers.append("unexpected_integrated_artifact_sha256")
    if artifact_bytes != M085R_INTEGRATED_ARTIFACT_BYTES:
        blockers.append("unexpected_integrated_artifact_size")
    if _value(body, summary, "integrated_diloco_smoke_status") != "passed":
    ...
    if (
        _value(body, summary, "optimization_fidelity")
        != "integrated_optimizer_protocol_smoke"
    ):
        blockers.append("optimization_fidelity_not_integrated_smoke")
    ...
    if not blockers:
        status: LambdaIntegratedDilocoSuccessStatus = (
            "remote_integrated_diloco_synthetic_success"
        )
    elif bool(report.get("launch_request_sent")):
        status = "remote_integrated_diloco_synthetic_partial"
    else:
        status = "remote_integrated_diloco_synthetic_failed"
    ...
    return LambdaIntegratedDilocoSuccessRecord(
        ...
        integrated_diloco_smoke_command_passed=passed[
            "integrated_diloco_smoke_command"
        ],
        integrated_diloco_smoke_status=_value(
            body, summary, "integrated_diloco_smoke_status"
        ),
        ...
        warnings=[
            "M086 records historical M085R billable work but performs no billable action",
            "M085R proves integrated optimizer/protocol smoke semantics, "
            "not full DiLoCo training or parameter-fragment synchronization",
        ],
    )


def load_lambda_integrated_diloco_success_record(
    path: str | Path,
) -> LambdaIntegratedDilocoSuccessRecord:
    return LambdaIntegratedDilocoSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_integrated_diloco_success_record(
    path: str | Path,
    record: LambdaIntegratedDilocoSuccessRecord,
) -> None:
    target = Path(path)
...
def _find_clean_post_summary(workdir: Path) -> Path | None:
    ...
    global_candidates = sorted(
        Path("/tmp").glob("decodilo-lambda-post-m085r-summary*.json"),
...
def _existing_artifact_paths(workdir: Path, post_summary: Path | None) -> dict[str, str]:
    candidates = {
        ...
        "source_bundle": Path("/tmp/decodilo-source-bundle-m085r.tar.gz"),
        "dependency_bundle": Path("/tmp/decodilo-dependency-bundle-m068w.tar.gz"),
        "source_bundle_validation": Path(
            "/tmp/decodilo-lambda-m085r-source-bundle-validation.json"
        ),
        "dependency_bundle_validation": Path(
            "/tmp/decodilo-lambda-m085r-dependency-bundle-validation.json"
        ),
        "command_manifest": Path("/tmp/decodilo-lambda-m085r-command-manifest.json"),
        "manifest_validation": Path(
            "/tmp/decodilo-lambda-m085r-manifest-validation.json"
        ),
        "plan": Path("/tmp/decodilo-lambda-m085r-plan.json"),
        "gate_check": Path("/tmp/decodilo-lambda-m085r-gate-check.json"),
        "authorization": Path(
            "/tmp/decodilo-lambda-m085r-integrated-diloco-authorization.json"
        ),
        "runbook_preview": Path(
            "/tmp/decodilo-lambda-m085r-integrated-diloco-runbook-preview.json"
        ),
    }
    return {
        name: str(path)
