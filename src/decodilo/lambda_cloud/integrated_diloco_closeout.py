"""M085R integrated synthetic DiLoCo closeout."""
...
class LambdaIntegratedDilocoCloseout(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M086"
    closeout_status: LambdaIntegratedDilocoCloseoutStatus
    closeout_succeeded: bool
    integrated_diloco_success: bool
    ...
    def _validate_closeout(self) -> LambdaIntegratedDilocoCloseout:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M086 closeout must not enable launch or spend")
...
def build_lambda_integrated_diloco_closeout_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
    evidence_package: str | Path,
) -> LambdaIntegratedDilocoCloseout:
    record = load_lambda_integrated_diloco_success_record(success_record)
    reconcile = load_lambda_integrated_diloco_reconciliation(reconciliation)
    package = load_lambda_integrated_diloco_evidence_package(evidence_package)
    blockers: list[str] = []
    ...
    if not reconcile.integrated_semantics_confirmed:
    ...
    warnings = [
        "M086 is offline; historical M085R billable action remains recorded separately",
        "M085R is integrated synthetic smoke, not full DiLoCo training",
    ]
    if closeout_succeeded and (record.warnings or package.warnings):
        status: LambdaIntegratedDilocoCloseoutStatus = "closed_with_warnings"
    elif closeout_succeeded:
        status = "closed_success"
    else:
        status = "unresolved"
    return LambdaIntegratedDilocoCloseout(
        closeout_status=status,
        closeout_succeeded=closeout_succeeded,
        integrated_diloco_success=(
            record.success_status == "remote_integrated_diloco_synthetic_success"
        ),
        ...
        integrated_semantics_confirmed=reconcile.integrated_semantics_confirmed,
...
def load_lambda_integrated_diloco_closeout(
    path: str | Path,
) -> LambdaIntegratedDilocoCloseout:
    return LambdaIntegratedDilocoCloseout.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_integrated_diloco_closeout(
    path: str | Path,
    report: LambdaIntegratedDilocoCloseout,
) -> None:
    target = Path(path)
