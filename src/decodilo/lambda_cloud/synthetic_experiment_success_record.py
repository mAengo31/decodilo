"""M077R remote bounded synthetic experiment success record."""
...
M077R_REQUIRED_STAGES = (
    ...
    "synthetic_experiment_command",
)
M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_PATH = (
    "/tmp/decodilo-synthetic-experiment.json"
)
M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_SHA256 = (
    "a65a62edbbf4b59288c3cea939d61cd03bda84b856c1b7328942400009590d34"
)
M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_BYTES = 1909
M077R_SOURCE_BUNDLE_SHA256 = (
    "ecd486e69f63749f548a70c7970c1cfe9c475f64d23661ae20c6f6f6743b8d29"
)
...
LambdaSyntheticExperimentSuccessStatus = Literal[
    "first_remote_synthetic_experiment_success",
    "first_remote_synthetic_experiment_partial",
    "first_remote_synthetic_experiment_failed",
]


class LambdaSyntheticExperimentSuccessRecord(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M078"
    success_status: LambdaSyntheticExperimentSuccessStatus
    ...
    synthetic_experiment_command_passed: bool
    synthetic_experiment_status: str | None = None
    ...
    def _validate_offline_record(self) -> LambdaSyntheticExperimentSuccessRecord:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M078 success record must remain offline and disabled")
        if (
            self.success_status == "first_remote_synthetic_experiment_success"
            and self.blockers
        ):
            raise ValueError("successful synthetic experiment record cannot carry blockers")
        return self
...
def build_lambda_synthetic_experiment_success_record_from_paths(
    *,
    workdir: str | Path,
) -> LambdaSyntheticExperimentSuccessRecord:
    ...
    synthetic_status = body.get("synthetic_experiment_status") or summary.get(
        "synthetic_experiment_status"
    )
    learner_check = body.get("learner_or_runtime_check_passed")
    update_check = body.get("update_or_commit_check_passed")
    ...
    if artifact_path != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_PATH:
        blockers.append("unexpected_synthetic_experiment_artifact_path")
    if artifact_sha != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_SHA256:
        blockers.append("unexpected_synthetic_experiment_artifact_sha256")
    if artifact_bytes != M077R_SYNTHETIC_EXPERIMENT_ARTIFACT_BYTES:
        blockers.append("unexpected_synthetic_experiment_artifact_size")
    if synthetic_status != "passed":
        blockers.append("synthetic_experiment_status_not_passed")
    if learner_check is not True:
        blockers.append("learner_or_runtime_check_not_passed")
    if update_check is not True:
    ...
    if not blockers:
        status: LambdaSyntheticExperimentSuccessStatus = (
            "first_remote_synthetic_experiment_success"
        )
    elif bool(report.get("launch_request_sent")):
        status = "first_remote_synthetic_experiment_partial"
    else:
        status = "first_remote_synthetic_experiment_failed"
    ...
    return LambdaSyntheticExperimentSuccessRecord(
        ...
        synthetic_experiment_command_passed=passed["synthetic_experiment_command"],
        synthetic_experiment_status=synthetic_status,
        learner_or_runtime_check_passed=learner_check,
        update_or_commit_check_passed=update_check,
        ...
        warnings=[
            "M078 records historical M077R billable work but performs no billable action",
        ],
    )


def load_lambda_synthetic_experiment_success_record(
    path: str | Path,
) -> LambdaSyntheticExperimentSuccessRecord:
    return LambdaSyntheticExperimentSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_synthetic_experiment_success_record(
    path: str | Path,
    record: LambdaSyntheticExperimentSuccessRecord,
) -> None:
    target = Path(path)
...
def _find_clean_post_summary(workdir: Path) -> Path | None:
    ...
    global_candidates = sorted(
        Path("/tmp").glob("decodilo-lambda-post-m077r-summary*.json"),
...
def _existing_artifact_paths(workdir: Path, post_summary: Path | None) -> dict[str, str]:
    candidates = {
        ...
        "source_bundle": Path("/tmp/decodilo-source-bundle-m077r.tar.gz"),
        "dependency_bundle": Path("/tmp/decodilo-dependency-bundle-m068w.tar.gz"),
        "source_bundle_validation": Path(
            "/tmp/decodilo-lambda-m077r-source-bundle-validation.json"
        ),
        "dependency_bundle_validation": Path(
            "/tmp/decodilo-lambda-m077r-dependency-bundle-validation.json"
        ),
        "command_manifest": Path("/tmp/decodilo-lambda-m077r-command-manifest.json"),
        "manifest_validation": Path(
            "/tmp/decodilo-lambda-m077r-manifest-validation.json"
        ),
        "plan": Path("/tmp/decodilo-lambda-m077r-plan.json"),
        "gate_check": Path("/tmp/decodilo-lambda-m077r-gate-check.json"),
    }
    return {
        name: str(path)
