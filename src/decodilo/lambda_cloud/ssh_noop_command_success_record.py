"""Success record for the completed M057 SSH no-op command run."""
...
LambdaSSHNoopCommandSuccessStatus = Literal[
    "ssh_noop_command_success",
    "ssh_noop_command_partial",
    "ssh_noop_command_failed",
]


class LambdaSSHNoopCommandSuccessRecord(BaseModel):
    ...
    ssh_attempted: bool
    ssh_auth_or_command_success: bool
    command_executed: bool
    command: str = "true"
    command_category: str = "noop"
    ...
    status: LambdaSSHNoopCommandSuccessStatus
    ...
    def _validate_closeout_only(self) -> LambdaSSHNoopCommandSuccessRecord:
        if (
            self.launch_ready
            ...
            raise ValueError("M058 success record cannot enable launch or mutation")
        if self.command != "true" or self.command_category != "noop":
            raise ValueError("M057 success record can only classify the true no-op")
        if self.status == "ssh_noop_command_success" and (
            self.stdout_stored
            ...
            raise ValueError("successful M057 no-op record cannot include forbidden work")
        return self
...
def build_lambda_ssh_noop_command_success_record_from_paths(
    *,
    workdir: str | Path,
    final_discovery: str | Path | None = None,
    secret_scan: str | Path | None = None,
) -> LambdaSSHNoopCommandSuccessRecord:
    workdir_path = Path(workdir)
    discovery_path = _resolve_final_discovery(final_discovery)
    ...
    secret_scan_passed = _secret_scan_passed(secret_scan)
    final_instance_count = len(discovery.instances)
    final_unmanaged_count = len(discovery.unmanaged_instances)
    blockers: list[str] = []
    if report.run_id != "lambda-m057-minimal-remote-command":
        blockers.append("m057_run_id_not_confirmed")
    ...
    if evidence.get("approved_command") != "true":
        blockers.append("approved_command_not_true")
    if evidence.get("command_output_collected") is not False:
        blockers.append("command_output_collection_not_false")
    if evidence.get("stdout_stored") is not False:
        blockers.append("stdout_stored")
    ...
    status: LambdaSSHNoopCommandSuccessStatus = (
        ...
        if not blockers
            ...
            else "ssh_noop_command_failed"
        )
    )
    return LambdaSSHNoopCommandSuccessRecord(
        ...
        ssh_auth_or_command_success=report.ssh_auth_result == "remote_command_succeeded",
        command_executed=report.remote_command_attempted,
        command_exit_code=report.ssh_exit_status,
        stdout_stored=bool(evidence.get("stdout_stored")),
        ...
        warnings=[
            "M058 records historical M057 billable action only",
            "M057 is classified as ssh_noop_command_success, not pure connectivity",
        ],
    )


def _resolve_final_discovery(final_discovery: str | Path | None) -> Path:
    candidates = []
    if final_discovery is not None:
        candidates.append(Path(final_discovery))
    candidates.extend(
        [
            Path("/tmp/decodilo-lambda-post-m057-final-clean-discovery.json"),
            Path("/tmp/decodilo-lambda-post-m057-discovery.json"),
        ]
    )
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError("M057 final discovery artifact missing")


def _secret_scan_passed(secret_scan: str | Path | None) -> bool:
    path = Path(secret_scan or "/tmp/decodilo-lambda-m057-secret-scan.txt")
    if not path.exists():
        return False
    text = path.read_text(encoding="utf-8", errors="ignore").strip()
    return text == "" or "NO_SECRET_PATTERN_MATCHES" in text


def load_lambda_ssh_noop_command_success_record(
    path: str | Path,
) -> LambdaSSHNoopCommandSuccessRecord:
    return LambdaSSHNoopCommandSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_ssh_noop_command_success_record(
    path: str | Path,
    record: LambdaSSHNoopCommandSuccessRecord,
) -> None:
    target = Path(path)
