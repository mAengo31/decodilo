"""M081R2 DiLoCo-shaped synthetic evidence package."""
...
class LambdaDilocoSyntheticEvidencePackage(BaseModel):
    ...
    def _validate_package(self) -> LambdaDilocoSyntheticEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M082 evidence package must remain offline and disabled")
...
def build_lambda_diloco_synthetic_evidence_package_from_paths(
    *,
    success_record: str | Path,
    reconciliation: str | Path,
) -> LambdaDilocoSyntheticEvidencePackage:
    ...
    return LambdaDilocoSyntheticEvidencePackage(
...
def load_lambda_diloco_synthetic_evidence_package(
    path: str | Path,
) -> LambdaDilocoSyntheticEvidencePackage:
    return LambdaDilocoSyntheticEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_diloco_synthetic_evidence_package(
    path: str | Path,
    report: LambdaDilocoSyntheticEvidencePackage,
) -> None:
    target = Path(path)
