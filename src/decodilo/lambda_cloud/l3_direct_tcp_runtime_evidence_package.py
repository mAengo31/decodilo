"""Offline evidence package for Lambda L3 direct TCP learner/syncer runtime."""
...
class LambdaL3DirectTcpRuntimeEvidencePackage(BaseModel):
    """Validated summary for the Lambda L3 remote multi-instance proof."""

    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "Lambda-L3"
    evidence_complete: bool
    lambda_l3_direct_tcp_runtime_passed: bool
    ...
    def _validate_boundaries(self) -> LambdaL3DirectTcpRuntimeEvidencePackage:
        if self.launch_ready or self.launch_allowed:
            raise ValueError("Lambda L3 evidence package cannot enable launch")
        if self.production_scale_ready:
            raise ValueError("Lambda L3 evidence package cannot claim production scale")
        if self.pathway_operation_layer_ready:
            raise ValueError("Lambda L3 evidence package cannot claim Pathway readiness")
...
def build_lambda_l3_direct_tcp_runtime_evidence_package_from_dir(
    evidence_dir: str | Path,
) -> LambdaL3DirectTcpRuntimeEvidencePackage:
    ...
    return LambdaL3DirectTcpRuntimeEvidencePackage(
        evidence_complete=evidence_complete,
        lambda_l3_direct_tcp_runtime_passed=evidence_complete,
        ...
        blockers=blockers,
        warnings=[
            "Lambda L3 proof uses real billable Lambda instances historically; parser is offline.",
            (
                "L3 proves direct TCP remote process/instance split, "
...
def load_lambda_l3_direct_tcp_runtime_evidence_package(
    path: str | Path,
) -> LambdaL3DirectTcpRuntimeEvidencePackage:
    return LambdaL3DirectTcpRuntimeEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_l3_direct_tcp_runtime_evidence_package(
    path: str | Path,
    package: LambdaL3DirectTcpRuntimeEvidencePackage,
) -> None:
    target = Path(path)
