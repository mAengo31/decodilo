"""Safe parser for parameter-fragment smoke JSON artifacts."""
...
ParameterFragmentArtifactParseStatus = Literal[
    "parsed_safe_parameter_fragment_smoke_artifact",
    "parsed_redacted_parameter_fragment_smoke_artifact",
...
class ParameterFragmentArtifactParserReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M087R"
    parse_status: ParameterFragmentArtifactParseStatus
    artifact_path: str
    declared_artifact_path: str = PARAMETER_FRAGMENT_SMOKE_DECLARED_ARTIFACT_PATH
    ...
    def _validate_report(self) -> ParameterFragmentArtifactParserReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("parameter-fragment artifact parser must remain offline")
        ...
        if (
            ...
            raise ValueError("parameter-fragment smoke cannot overclaim model fragments")
        if (
            self.parsed_summary
            and self.parsed_summary.get("overlap_semantics") == "implemented"
        ):
            raise ValueError("parameter-fragment smoke cannot overclaim overlap")
        if (
            self.parsed_summary
            and self.parsed_summary.get("quantization_semantics") == "implemented"
        ):
            raise ValueError("parameter-fragment smoke cannot overclaim quantization")
        return self
...
def _summary_from_body(body: dict[str, Any], redactions: list[str]) -> dict[str, Any]:
    allowed_keys = [
        "parameter_fragment_smoke_status",
        ...
        "fragment_versions_before",
        "fragment_versions_after",
...
def parse_parameter_fragment_artifact_file(
    *,
    artifact_path: str | Path,
    policy: str | Path | dict[str, Any] | Any,
) -> ParameterFragmentArtifactParserReport:
    ...
    if _policy_value(loaded_policy, "policy_status", "policy_defined") not in {
        ...
        return ParameterFragmentArtifactParserReport(
    ...
    if secret_scan_passed:
        return ParameterFragmentArtifactParserReport(
            parse_status="parsed_safe_parameter_fragment_smoke_artifact",
            artifact_path=str(path),
            declared_artifact_path=declared_path,
            artifact_exists=True,
            artifact_bytes=size,
    ...
    return ParameterFragmentArtifactParserReport(
        parse_status="parsed_redacted_parameter_fragment_smoke_artifact",
        artifact_path=str(path),
        declared_artifact_path=declared_path,
        artifact_exists=True,
        artifact_bytes=size,
...
def load_parameter_fragment_artifact_parser_report(
    path: str | Path,
) -> ParameterFragmentArtifactParserReport:
    return ParameterFragmentArtifactParserReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_parameter_fragment_artifact_parser_report(
    path: str | Path,
    report: ParameterFragmentArtifactParserReport,
) -> None:
    target = Path(path)
