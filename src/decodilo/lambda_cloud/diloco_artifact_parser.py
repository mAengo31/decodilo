"""Safe parser for DiLoCo-shaped synthetic smoke JSON artifacts."""
...
DilocoArtifactParseStatus = Literal[
    "parsed_safe_diloco_smoke_artifact",
    "parsed_redacted_diloco_smoke_artifact",
...
class DilocoArtifactParserReport(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M081S"
    parse_status: DilocoArtifactParseStatus
    artifact_path: str
    ...
    def _validate_report(self) -> DilocoArtifactParserReport:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("DiLoCo artifact parser must remain offline")
        ...
        if (
            self.parsed_summary
            and self.parsed_summary.get("optimization_fidelity") == "full_diloco"
            and self.parsed_summary.get("inner_optimizer_semantics") != "adamw"
        ):
            raise ValueError("full_diloco summary requires true inner AdamW semantics")
        return self
...
def parse_diloco_artifact_file(
    *,
    artifact_path: str | Path,
    policy: str | Path | dict[str, Any] | Any,
) -> DilocoArtifactParserReport:
    ...
    if _policy_value(loaded_policy, "policy_status", "policy_defined") not in {
        ...
        return DilocoArtifactParserReport(
    ...
    if secret_scan_passed:
        return DilocoArtifactParserReport(
            parse_status="parsed_safe_diloco_smoke_artifact",
            artifact_path=str(path),
            declared_artifact_path=declared_path,
            artifact_exists=True,
            artifact_bytes=size,
    ...
    return DilocoArtifactParserReport(
        parse_status="parsed_redacted_diloco_smoke_artifact",
        artifact_path=str(path),
        declared_artifact_path=declared_path,
        artifact_exists=True,
        artifact_bytes=size,
...
def load_diloco_artifact_parser_report(path: str | Path) -> DilocoArtifactParserReport:
    return DilocoArtifactParserReport.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_diloco_artifact_parser_report(
    path: str | Path,
    report: DilocoArtifactParserReport,
) -> None:
    target = Path(path)
