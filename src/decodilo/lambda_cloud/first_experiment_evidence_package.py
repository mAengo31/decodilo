"""M071R first remote Decodilo experiment evidence package."""
...
class LambdaFirstExperimentEvidencePackage(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M072"
    evidence_complete: bool
    first_experiment_success: bool
    ...
    def _validate_package(self) -> LambdaFirstExperimentEvidencePackage:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M072 evidence package must remain offline and disabled")
...
def build_lambda_first_experiment_evidence_package_from_paths(
    *,
    ...
    return LambdaFirstExperimentEvidencePackage(
        evidence_complete=evidence_complete,
        first_experiment_success=record.status == "first_experiment_runtime_success",
...
def load_lambda_first_experiment_evidence_package(
    path: str | Path,
) -> LambdaFirstExperimentEvidencePackage:
    return LambdaFirstExperimentEvidencePackage.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_first_experiment_evidence_package(
    path: str | Path,
    report: LambdaFirstExperimentEvidencePackage,
) -> None:
    target = Path(path)
