"""M079R2 remote learner/syncer synthetic smoke success record."""
...
M079R2_REQUIRED_STAGES = (
    ...
    "learner_syncer_smoke_command",
)
M079R2_LEARNER_SYNCER_ARTIFACT_PATH = "/tmp/decodilo-learner-syncer-smoke.json"
M079R2_LEARNER_SYNCER_ARTIFACT_SHA256 = (
    "d493db8a521b0e8a07a5429801a547a7649840cbe78525d73f0530c67bb10f7e"
)
M079R2_LEARNER_SYNCER_ARTIFACT_BYTES = 2725
M079R2_SOURCE_BUNDLE_SHA256 = (
    "754ec32bafb9da82516403f8c9bc66e94692a6b80e72055163e9ba6932566674"
)
...
LambdaLearnerSyncerSmokeSuccessStatus = Literal[
    "remote_learner_syncer_smoke_success",
    "remote_learner_syncer_smoke_partial",
    "remote_learner_syncer_smoke_failed",
]


class LambdaLearnerSyncerSmokeSuccessRecord(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M080"
    success_status: LambdaLearnerSyncerSmokeSuccessStatus
    ...
    synthetic_steps_completed: int | None = None
    synthetic_updates_accepted: int | None = None
    ...
    def _validate_offline_record(self) -> LambdaLearnerSyncerSmokeSuccessRecord:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M080 success record must remain offline and disabled")
        if self.success_status == "remote_learner_syncer_smoke_success" and self.blockers:
            raise ValueError("successful learner/syncer smoke record cannot carry blockers")
        return self
...
def build_lambda_learner_syncer_smoke_success_record_from_paths(
    *,
    workdir: str | Path,
) -> LambdaLearnerSyncerSmokeSuccessRecord:
    ...
    learner_status = body.get("learner_syncer_smoke_status") or summary.get(
    ...
    if artifact_path != M079R2_LEARNER_SYNCER_ARTIFACT_PATH:
        blockers.append("unexpected_learner_syncer_artifact_path")
    if artifact_sha != M079R2_LEARNER_SYNCER_ARTIFACT_SHA256:
        blockers.append("unexpected_learner_syncer_artifact_sha256")
    if artifact_bytes != M079R2_LEARNER_SYNCER_ARTIFACT_BYTES:
        blockers.append("unexpected_learner_syncer_artifact_size")
    if learner_status != "passed":
        blockers.append("learner_syncer_smoke_status_not_passed")
    for key in (
        "learner_check_passed",
        "syncer_check_passed",
        "learner_syncer_exchange_check_passed",
        "update_or_commit_check_passed",
        "replay_or_metric_check_passed",
    ):
        if (body.get(key) if key in body else summary.get(key)) is not True:
    ...
    if not blockers:
        status: LambdaLearnerSyncerSmokeSuccessStatus = "remote_learner_syncer_smoke_success"
    elif bool(report.get("launch_request_sent")):
        status = "remote_learner_syncer_smoke_partial"
    else:
        status = "remote_learner_syncer_smoke_failed"
    ...
    return LambdaLearnerSyncerSmokeSuccessRecord(
        ...
        source_bundle_sha256=(
            ...
            or M079R2_SOURCE_BUNDLE_SHA256
        ),
        ...
        learner_syncer_smoke_command_passed=passed["learner_syncer_smoke_command"],
        learner_syncer_smoke_status=learner_status,
        learner_check_passed=value("learner_check_passed"),
        syncer_check_passed=value("syncer_check_passed"),
        learner_syncer_exchange_check_passed=value(
            "learner_syncer_exchange_check_passed"
        ),
        update_or_commit_check_passed=value("update_or_commit_check_passed"),
        replay_or_metric_check_passed=value("replay_or_metric_check_passed"),
        synthetic_steps_completed=_int_value(value("synthetic_steps_completed")),
        synthetic_updates_accepted=_int_value(value("synthetic_updates_accepted")),
        sync_rounds_completed=_int_value(value("sync_rounds_completed")),
        ...
        warnings=[
            "M080 records historical M079R2 billable work but performs no billable action",
        ],
    )


def load_lambda_learner_syncer_smoke_success_record(
    path: str | Path,
) -> LambdaLearnerSyncerSmokeSuccessRecord:
    return LambdaLearnerSyncerSmokeSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_learner_syncer_smoke_success_record(
    path: str | Path,
    record: LambdaLearnerSyncerSmokeSuccessRecord,
) -> None:
    target = Path(path)
...
def _find_clean_post_summary(workdir: Path) -> Path | None:
    ...
    global_candidates = sorted(
        Path("/tmp").glob("decodilo-lambda-post-m079r2-summary*.json"),
...
def _existing_artifact_paths(workdir: Path, post_summary: Path | None) -> dict[str, str]:
    candidates = {
        ...
        "source_bundle": Path("/tmp/decodilo-source-bundle-m079r2.tar.gz"),
        "dependency_bundle": Path("/tmp/decodilo-dependency-bundle-m068w.tar.gz"),
        "source_bundle_validation": Path(
            "/tmp/decodilo-lambda-m079r2-source-bundle-validation.json"
        ),
        "dependency_bundle_validation": Path(
            "/tmp/decodilo-lambda-m079r2-dependency-bundle-validation.json"
        ),
        "command_manifest": Path("/tmp/decodilo-lambda-m079r2-command-manifest.json"),
        "manifest_validation": Path(
            "/tmp/decodilo-lambda-m079r2-manifest-validation.json"
        ),
        "plan": Path("/tmp/decodilo-lambda-m079r2-plan.json"),
        "gate_check": Path("/tmp/decodilo-lambda-m079r2-gate-check.json"),
        "declared_artifact_policy": Path(
            "/tmp/decodilo-lambda-m079r-declared-artifact-policy.json"
        ),
    }
    return {
        name: str(path)
