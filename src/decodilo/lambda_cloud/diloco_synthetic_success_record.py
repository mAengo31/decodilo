"""M081R2 remote DiLoCo-shaped synthetic success record."""
...
M081R2_REQUIRED_STAGES = (
    ...
    "diloco_smoke_command",
)
M081R2_DILOCO_ARTIFACT_PATH = "/tmp/decodilo-diloco-smoke.json"
M081R2_DILOCO_ARTIFACT_SHA256 = (
    "7e42f2058879dc07853f53c77ffc6729a4c0e9cab23b3b9652c07abb63efce53"
)
M081R2_DILOCO_ARTIFACT_BYTES = 3355
M081R2_SOURCE_BUNDLE_SHA256 = (
    "8bc0c5c3f6436ea0fcc9467291d56ec5756b724a09a10db605c6321cce7b82df"
)
...
LambdaDilocoSyntheticSuccessStatus = Literal[
    "remote_diloco_shaped_synthetic_success",
    "remote_diloco_shaped_synthetic_partial",
    "remote_diloco_shaped_synthetic_failed",
]


class LambdaDilocoSyntheticSuccessRecord(BaseModel):
    model_config = ConfigDict(frozen=True)

    report_schema_version: int = 1
    milestone: str = "M082"
    success_status: LambdaDilocoSyntheticSuccessStatus
    ...
    learner_count_observed: int | None = None
    ...
    def _validate_offline_record(self) -> LambdaDilocoSyntheticSuccessRecord:
        if self.launch_ready or self.launch_allowed or self.billable_action_performed:
            raise ValueError("M082 success record must remain offline and disabled")
        if (
            self.success_status == "remote_diloco_shaped_synthetic_success"
            and self.blockers
        ):
            raise ValueError("successful DiLoCo synthetic record cannot carry blockers")
        return self
...
def build_lambda_diloco_synthetic_success_record_from_paths(
    *,
    workdir: str | Path,
) -> LambdaDilocoSyntheticSuccessRecord:
    ...
    if artifact_path != M081R2_DILOCO_ARTIFACT_PATH:
        blockers.append("unexpected_diloco_artifact_path")
    if artifact_sha != M081R2_DILOCO_ARTIFACT_SHA256:
        blockers.append("unexpected_diloco_artifact_sha256")
    if artifact_bytes != M081R2_DILOCO_ARTIFACT_BYTES:
        blockers.append("unexpected_diloco_artifact_size")
    if _value(body, summary, "diloco_smoke_status") != "passed":
    ...
    if _value(body, summary, "optimization_fidelity") != "diloco_shaped_protocol_only":
        blockers.append("optimization_fidelity_overclaimed_or_missing")
    if _value(body, summary, "inner_optimizer_semantics") != "synthetic_placeholder":
        blockers.append("inner_optimizer_semantics_unexpected")
    if _value(body, summary, "outer_optimizer_semantics") != "token_weighted_merge":
    ...
    if not blockers:
        status: LambdaDilocoSyntheticSuccessStatus = (
            "remote_diloco_shaped_synthetic_success"
        )
    elif bool(report.get("launch_request_sent")):
        status = "remote_diloco_shaped_synthetic_partial"
    else:
        status = "remote_diloco_shaped_synthetic_failed"
    ...
    return LambdaDilocoSyntheticSuccessRecord(
        ...
        source_bundle_sha256=(
            ...
            or M081R2_SOURCE_BUNDLE_SHA256
        ),
        ...
        diloco_smoke_command_passed=passed["diloco_smoke_command"],
        diloco_smoke_status=_value(body, summary, "diloco_smoke_status"),
        ...
        diloco_shape_check_passed=_value(body, summary, "diloco_shape_check_passed"),
        learner_syncer_exchange_check_passed=_value(
            body, summary, "learner_syncer_exchange_check_passed"
        ),
        update_or_commit_check_passed=_value(body, summary, "update_or_commit_check_passed"),
        replay_or_metric_check_passed=_value(
            body, summary, "replay_or_metric_check_passed"
        ),
        learner_count_observed=_int_value(_value(body, summary, "learner_count_observed")),
        sync_rounds_completed=_int_value(_value(body, summary, "sync_rounds_completed")),
        synthetic_updates_accepted=_int_value(
            _value(body, summary, "synthetic_updates_accepted")
        ),
        ...
        warnings=[
            "M082 records historical M081R2 billable work but performs no billable action",
            "DiLoCo optimizer fidelity is not overclaimed by this protocol-shaped smoke",
        ],
    )


def load_lambda_diloco_synthetic_success_record(
    path: str | Path,
) -> LambdaDilocoSyntheticSuccessRecord:
    return LambdaDilocoSyntheticSuccessRecord.model_validate_json(
        Path(path).read_text(encoding="utf-8")
    )


def write_lambda_diloco_synthetic_success_record(
    path: str | Path,
    record: LambdaDilocoSyntheticSuccessRecord,
) -> None:
    target = Path(path)
...
def _find_clean_post_summary(workdir: Path) -> Path | None:
    ...
    global_candidates = sorted(
        Path("/tmp").glob("decodilo-lambda-post-m081r2-summary*.json"),
...
def _existing_artifact_paths(workdir: Path, post_summary: Path | None) -> dict[str, str]:
    candidates = {
        ...
        "source_bundle": Path("/tmp/decodilo-source-bundle-m081r2.tar.gz"),
        "dependency_bundle": Path("/tmp/decodilo-dependency-bundle-m068w.tar.gz"),
        "source_bundle_validation": Path(
            "/tmp/decodilo-lambda-m081r2-source-bundle-validation.json"
        ),
        "dependency_bundle_validation": Path(
            "/tmp/decodilo-lambda-m081r2-dependency-bundle-validation.json"
        ),
        "command_manifest": Path("/tmp/decodilo-lambda-m081r2-command-manifest.json"),
        "manifest_validation": Path(
            "/tmp/decodilo-lambda-m081r2-manifest-validation.json"
        ),
        "plan": Path("/tmp/decodilo-lambda-m081r2-plan.json"),
        "gate_check": Path("/tmp/decodilo-lambda-m081r2-gate-check.json"),
        "manifest_artifact_policy": Path(
            "/tmp/decodilo-lambda-m081r-manifest-artifact-policy.json"
        ),
    }
    return {
        name: str(path)
