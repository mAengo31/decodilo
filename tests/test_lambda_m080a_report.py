...
from decodilo.lambda_cloud.m080a_report import build_lambda_m080a_report_from_paths
...
def test_m080a_report_passes_when_m081r_future_authorized(tmp_path):
    ...
    report = build_lambda_m080a_report_from_paths(
        command_discovery=discovery_path,
        policy=policy_path,
        authorization=authorization_path,
    ...
    assert report.diloco_smoke_command_added is True
    assert report.discovery_status == "found_safe_diloco_synthetic_command"
    assert report.policy_status == "policy_passed"
    assert (
        report.m081r_authorization_status
