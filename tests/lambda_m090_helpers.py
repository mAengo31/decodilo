...
def make_m089r_workdir(tmp_path: Path) -> Path:
    workdir = tmp_path / "m089r"
    workdir.mkdir()
    stage_results = [
        {
            "stage": stage,
    ...
    artifact_body = {
        ...
        "integrated_reference_check_passed": True,
        "protocol_optimizer_link_check_passed": True,
        "optimizer_fragment_link_check_passed": True,
        "protocol_fragment_link_check_passed": True,
        "max_abs_error": 0.0,
        "full_diloco_training_claimed": False,
        "real_model_training_claimed": False,
        "true_model_fragment_claimed": False,
    ...
    base = {
        "run_id": "lambda-m089r-bounded-diloco-experiment",
        "selected_candidate": "gpu_1x_a10",
        "selected_shape": "gpu_1x_a10",
        "selected_region": "us-east-1",
        "source_bundle_sha256": M089R_SOURCE_BUNDLE_SHA256,
        ...
        "experiment_output_artifact_path": M089R_BOUNDED_DILOCO_ARTIFACT_PATH,
        "experiment_output_artifact_bytes": M089R_BOUNDED_DILOCO_ARTIFACT_BYTES,
        "experiment_output_artifact_sha256": M089R_BOUNDED_DILOCO_ARTIFACT_SHA256,
        ...
        "estimated_spend": 0.09480059634388115,
        "launch_ready": False,
        "launch_allowed": False,
    }
    evidence = {
    ...
    _write_json(
        ...
        {
            "billable_action_performed": True,
            "estimated_spend": 7.348883437510166,
...
def write_m090_bounded_closeout_chain(
    tmp_path: Path,
    workdir: Path,
) -> dict[str, Path]:
    from decodilo.lambda_cloud.bounded_diloco_experiment_artifact_audit import (
        ...
        write_lambda_bounded_diloco_experiment_artifact_audit,
    )
    from decodilo.lambda_cloud.bounded_diloco_experiment_closeout import (
        ...
        write_lambda_bounded_diloco_experiment_closeout,
    )
    from decodilo.lambda_cloud.bounded_diloco_experiment_evidence_package import (
        ...
        write_lambda_bounded_diloco_experiment_evidence_package,
    )
    from decodilo.lambda_cloud.bounded_diloco_experiment_reconciliation import (
        ...
        write_lambda_bounded_diloco_experiment_reconciliation,
    )
    from decodilo.lambda_cloud.bounded_diloco_experiment_success_record import (
        build_lambda_bounded_diloco_experiment_success_record_from_paths,
        write_lambda_bounded_diloco_experiment_success_record,
    )

    success_path = tmp_path / "bounded-success.json"
    reconciliation_path = tmp_path / "bounded-reconciliation.json"
    evidence_path = tmp_path / "bounded-evidence.json"
    closeout_path = tmp_path / "bounded-closeout.json"
    audit_path = tmp_path / "bounded-audit.json"
    write_lambda_bounded_diloco_experiment_success_record(
        success_path,
        build_lambda_bounded_diloco_experiment_success_record_from_paths(
            workdir=workdir,
...
def write_scaffold_decision(tmp_path: Path) -> Path:
    path = tmp_path / "scaffold-decision.json"
    _write_json(
        path,
        {
            ...
            "completed_layers": [
                "remote runtime/protocol smoke",
                "remote learner/syncer synthetic smoke",
                "remote DiLoCo-shaped protocol smoke",
                "remote optimizer-fidelity smoke",
                "remote integrated protocol+optimizer smoke",
                "remote parameter-fragment synthetic smoke",
            ],
            "next_phase": "bounded_synthetic_diloco_experiment",
            "no_more_independent_smoke_categories_by_default": True,
            "future_live_work_should_run_bounded_experiment_manifests": True,
            "future_scaffold_exception_requires_concrete_failure": True,
            "blockers": [],
            "warnings": [],
            "launch_ready": False,
            "launch_allowed": False,
