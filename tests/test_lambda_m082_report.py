...
from decodilo.lambda_cloud.diloco_synthetic_artifact_audit import (
    ...
    write_lambda_diloco_synthetic_artifact_audit,
)
from decodilo.lambda_cloud.m082_report import build_lambda_m082_report_from_paths
...
def test_m082_report_passes_closeout_and_blocks_m083r_without_command(tmp_path):
    ...
    report = build_lambda_m082_report_from_paths(
        success_record=paths["success"],
        reconciliation=paths["reconciliation"],
        closeout=paths["closeout"],
        artifact_audit=artifact_audit_path,
        optimizer_readiness=readiness_path,
        optimizer_discovery=discovery_path,
        optimizer_policy=policy_path,
        authorization=authorization_path,
        runbook_preview=runbook_path,
    )

    assert report.report_passed is True
    assert report.diloco_synthetic_success_status == (
        "remote_diloco_shaped_synthetic_success"
    )
    assert report.optimization_fidelity == "diloco_shaped_protocol_only"
    assert report.m083r_authorization_status == "not_authorized"
    assert "no_safe_diloco_optimizer_command_found" in report.m083r_blockers
