...
from lambda_m078_helpers import make_m077r_workdir
...
def test_synthetic_experiment_success_record_classifies_m077r_success(tmp_path):
    workdir = make_m077r_workdir(tmp_path)

    record = build_lambda_synthetic_experiment_success_record_from_paths(
        workdir=workdir,
    )

    assert record.success_status == "first_remote_synthetic_experiment_success"
    assert record.infrastructure_passed is True
    assert record.synthetic_experiment_command_passed is True
    assert record.synthetic_experiment_status == "passed"
    assert record.learner_or_runtime_check_passed is True
