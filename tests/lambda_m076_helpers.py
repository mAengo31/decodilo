...
def make_m075r4_workdir(tmp_path: Path) -> Path:
    workdir = tmp_path / "m075r4"
    workdir.mkdir()
    stage_results = [
        {
            "stage": stage,
    ...
    artifact_body = {
        ...
        "protocol_or_event_check_passed": True,
    ...
    base = {
        "run_id": "lambda-m075r4-runtime-smoke",
        "selected_candidate": "gpu_1x_a10",
        "selected_shape": "gpu_1x_a10",
        "selected_region": "us-east-1",
        "source_bundle_sha256": M075R4_SOURCE_BUNDLE_SHA256,
        ...
        "experiment_output_artifact_path": M075R4_RUNTIME_SMOKE_ARTIFACT_PATH,
        "experiment_output_artifact_bytes": M075R4_RUNTIME_SMOKE_ARTIFACT_BYTES,
        "experiment_output_artifact_sha256": M075R4_RUNTIME_SMOKE_ARTIFACT_SHA256,
        ...
        "estimated_spend": 0.09355955439700435,
        "launch_ready": False,
        "launch_allowed": False,
    }
    ...
    spend = {
        "billable_action_performed": True,
        "estimated_spend": 7.252678635426693,
...
def write_m076_closeout_chain(tmp_path: Path, workdir: Path) -> dict[str, Path]:
    from decodilo.lambda_cloud.runtime_smoke_closeout import (
        build_lambda_runtime_smoke_closeout_from_paths,
        write_lambda_runtime_smoke_closeout,
    )
    from decodilo.lambda_cloud.runtime_smoke_evidence_package import (
        build_lambda_runtime_smoke_evidence_package_from_paths,
        write_lambda_runtime_smoke_evidence_package,
    )
    from decodilo.lambda_cloud.runtime_smoke_reconciliation import (
        build_lambda_runtime_smoke_reconciliation_from_paths,
        write_lambda_runtime_smoke_reconciliation,
    )
    from decodilo.lambda_cloud.runtime_smoke_success_record import (
        build_lambda_runtime_smoke_success_record_from_paths,
        write_lambda_runtime_smoke_success_record,
    )

    success_path = tmp_path / "runtime-smoke-success.json"
    reconciliation_path = tmp_path / "runtime-smoke-reconciliation.json"
    evidence_path = tmp_path / "runtime-smoke-evidence.json"
    closeout_path = tmp_path / "runtime-smoke-closeout.json"
    write_lambda_runtime_smoke_success_record(
        success_path,
        build_lambda_runtime_smoke_success_record_from_paths(workdir=workdir),
    )
    write_lambda_runtime_smoke_reconciliation(
        reconciliation_path,
        build_lambda_runtime_smoke_reconciliation_from_paths(
