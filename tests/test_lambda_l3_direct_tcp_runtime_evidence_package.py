...
def _write_minimal_l3_evidence(root: Path) -> None:
...
def test_lambda_l3_direct_tcp_runtime_evidence_package_accepts_distinct_instances(
    tmp_path: Path,
) -> None:
    _write_minimal_l3_evidence(tmp_path)

    package = build_lambda_l3_direct_tcp_runtime_evidence_package_from_dir(tmp_path)

    assert package.evidence_complete is True
    assert package.lambda_l3_direct_tcp_runtime_passed is True
...
def test_lambda_l3_direct_tcp_runtime_evidence_package_rejects_collocated_roles(
    tmp_path: Path,
) -> None:
    _write_minimal_l3_evidence(tmp_path)
    ...
    assert package.lambda_l3_direct_tcp_runtime_passed is False
