...
def test_dev_runtime_smoke_cli_writes_bounded_report(tmp_path):
...
def test_dev_runtime_smoke_cli_invalid_steps_exits_nonzero(tmp_path):
    ...
    assert report["safe_error_message"] == (
        "runtime smoke currently requires --max-steps 1"
    )
    assert report["network_used"] is False
    assert report["package_install_attempted"] is False
    assert report["download_attempted"] is False
