...
def test_m083r_authorization_not_authorized_without_optimizer_command(tmp_path):
    ...
    authorization = build_lambda_m083r_diloco_optimizer_authorization_from_paths(
        diloco_synthetic_closeout=paths["closeout"],
    ...
    assert "no_safe_diloco_optimizer_command_found" in authorization.blockers
...
def test_m083r_authorization_allows_future_when_optimizer_command_is_safe(tmp_path):
    workdir = make_m081r2_workdir(tmp_path)
    paths = write_m082_closeout_chain(tmp_path, workdir)
    readiness_path = tmp_path / "readiness.json"
    discovery_path = tmp_path / "discovery.json"
